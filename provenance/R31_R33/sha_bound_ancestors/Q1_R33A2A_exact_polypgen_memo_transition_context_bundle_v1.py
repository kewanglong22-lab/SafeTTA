...
R31B2_FINAL = R31B2_DIR / "R31B2_FINAL_LOCK.json"
...
DEFAULT_OUT = ROOT / "R33A2A_exact_polypgen_memo_transition_context_bundle_v1"
...
def verify_historical_files() -> Dict[str, Any]:
    require_sha(R31B2_FINAL, EXPECTED_R31B2_FINAL_SHA256, "R31B2 final")
    ...
    r31b2 = json.loads(R31B2_FINAL.read_text(encoding="utf-8"))
    if r31b2.get("status") not in {
        "PASS_R31B2_800CASE_PRE_GT_LOCK_COMPLETE",
        "PASS_R31B2_800CASE_MEMO_PREDICTION_AND_SEMANTIC_LOCK_COMPLETE",
    }:
        # Preserve strictness but show exact observed value if historical key differs.
        raise RuntimeError(
            f"Unexpected R31B2 final status={r31b2.get('status')!r}"
        )

    r31b1 = json.loads(R31B1_FINAL.read_text(encoding="utf-8"))
    ...
    return {
        ...
        "observed_script_sha256": {
            ...
            "R31B1": sha256_file(R31B1_SCRIPT),
...
def main() -> int:
    ...
    script_targets = [
        ...
        ("R31B1_MEMO_LR_SELECTION", R31B1_SCRIPT),
    ...
    lock_summaries = [
        extract_final_lock_summary(R31B2_FINAL, "R31B2"),
        extract_final_lock_summary(R31B1_FINAL, "R31B1"),
