...
VERSION = "2026-09-12-R33A2A-v1-fix1"
...
DEFAULT_OUT = ROOT / "R33A2A_exact_polypgen_memo_transition_context_bundle_v1_fix1"
...
def verify_historical_files() -> Dict[str, Any]:
    ...
    for p in [R31B2_SCRIPT, R22A1_SCRIPT, R31B1_SCRIPT]:
        if not p.is_file():
...
def main() -> int:
    ...
    print("FIX1                        : historical final-lock discovery by exact SHA")
    ...
    audit = {
        ...
        "version": VERSION,
        "implementation_fix": (
            "Mechanical historical-artifact resolution: locate R31B2 final "
            "lock by frozen SHA256 instead of assumed filename; R31B1 JSON "
            "metadata is non-gating provenance."
        ),
