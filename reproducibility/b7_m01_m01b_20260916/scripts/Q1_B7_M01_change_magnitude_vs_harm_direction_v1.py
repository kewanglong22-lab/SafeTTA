...
OUT_DIR = ROOT / "B7_M01_change_magnitude_vs_harm_direction_v1"
...
PASS_GATE = "PASS_B7_M01_CHANGE_MAGNITUDE_VS_HARM_DIRECTION_COMPLETE"
...
def clustered_bootstrap(d: pd.DataFrame, q: np.ndarray) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    samples = np.asarray(sorted(d["sample_id"].unique()))
    idx_map = {
...
def main() -> None:
    ...
    # Support gate: primary point analysis should have all 4 source-defined strata
    # with >=10 HARM and >=10 BENEFIT.
    support = (
        strata[strata["score"] == PRIMARY_FULL]
        .set_index("stratum")["support_gate"]
        .to_dict()
    )
    if not all(bool(support.get(k, False)) for k in [1, 2, 3, 4]):
        raise RuntimeError(
            f"Primary magnitude-stratum support insufficient: {support}. "
            "Do not change bins after outcome reveal."
        )
    ...
    if len(primary_row) != 1:
        raise RuntimeError("Primary M01 paired delta row missing.")
