...
Fix2 repairs repeated historical OOF outcome rows by exact-key consensus collapse; Fix1 schema/path binding is retained.\n\nThis is the first scientific P01B experiment after:
...
VERSION = "2026-09-15-B6-P01B-GEOM-CORE-v1-fix2"
...
OUT_DIR = ROOT / "B6_P01B_polypgen_geometry_core_experiment_v1_fix2"
...
def choose_target_outcomes(inv: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    if len(diag):
        show_cols = [
            "raw_rows", "rows", "samples", "states", "unique_keys",
            "duplicate_collapse_performed",
            "duplicate_multiplicity_min", "duplicate_multiplicity_median",
            "duplicate_multiplicity_max",
