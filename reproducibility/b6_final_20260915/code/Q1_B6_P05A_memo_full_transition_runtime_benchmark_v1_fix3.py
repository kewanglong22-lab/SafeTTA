...
VERSION = "2026-09-15-B6-P05A-v1-fix3"

ROOT = Path(r"F:\MEDSEG_SAFETTA")
CODE = ROOT / "code"

PROTOCOL_PATH = CODE / "B6_P05A_MEMO_FULL_TRANSITION_RUNTIME_PROTOCOL_v1_fix1.json"
EXPECTED_PROTOCOL_SHA256 = "895da6d38a423ea38034f84a92aeab06aad54bbc65f20bf32193f87d53bfd823"
...
OUT_DIR = ROOT / "B6_P05A_memo_full_transition_runtime_benchmark_v1_fix3"
...
def verify_r33_semantic_fingerprint(path: Path) -> Dict[str, Any]:
    """
    Extra static guard on top of the repaired byte SHA.
    This does not load models or execute inference.
    """
    ...
    required_functions = {
        ...
        "build_deeplab_runtime",
        "run_state_memo",
        "run_exact_semantic_lock",
        "score_target",
    }
    missing = sorted(required_functions - funcs)
    if missing:
        raise RuntimeError(f"R33A2B semantic fingerprint missing functions={missing}")
    ...
    missing_literals = [x for x in required_literals if x not in src]
    if missing_literals:
        raise RuntimeError(
            f"R33A2B semantic fingerprint missing literals={missing_literals}"
        )
...
def verify_protocol() -> Dict[str, Any]:
    ...
    if p.get("status") != "FROZEN_BEFORE_RUNTIME_MEASUREMENT_IMPLEMENTATION_FIX1":
        raise RuntimeError("P05A fix1 protocol status changed.")
...
def head_score(x130: np.ndarray, head: Dict[str, np.ndarray]) -> float:
    ...
    logit = float((xs @ head["lr_coef"].reshape(130, 1) + head["lr_intercept"][0]).item())
    if logit >= 0:
...
def main():
    ...
    r33_sha = require_sha(R33_SCRIPT, EXPECTED_R33_SCRIPT_SHA256, "R33A2B fix1")
    r33_fingerprint = verify_r33_semantic_fingerprint(R33_SCRIPT)
    ...
    print("R33A2B SHA256   =", r33_sha)
    print("R33 fingerprint =", r33_fingerprint["pass"])
