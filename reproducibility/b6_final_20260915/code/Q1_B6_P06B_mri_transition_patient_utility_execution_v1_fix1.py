...
- re-counts SOURCE/candidate TP/FP/FN from frozen FinalB1 packed masks
  against the already revealed/frozen CM6 PROMISE12 packed GT, decoding
  FinalB1 predictions with big bitorder and CM6 GT with little bitorder;
...
VERSION = "2026-09-15-B6-P06B-v1-fix1"
...
OUT_DIR = ROOT / "B6_P06B_mri_transition_patient_utility_execution_v1_fix1"
...
# Exact local producer-code lineage observed in B6-P06B1.
...
EXPECTED_FINALB1_SCRIPT_SHA256 = (
    "84fb53ca04ef6ad1c0c1e5138fd11767fb4aebfab8e0a072cd2b16d8f55060b7"
)
EXPECTED_FINALB3_SCRIPT_SHA256 = (
    "d1215ab90dd377f05929613d957299bf6c5ab55d67bec1cb33c572434354f042"
)
EXPECTED_CM6_SCRIPT_SHA256 = (
    "68208034e3f9ff5937bf44973f7882d0d2820073d9188e0cc07637f1963d24b0"
)

# FinalB1 serializes current-runtime prediction masks with BITORDER="big".
# CM6 serializes PROMISE12 GT with bitorder="little".
# These arrays MUST be decoded with their own producer conventions before
# pixelwise TP/FP/FN counting. Direct packed-byte AND is invalid across them.
PRED_MASK_BITORDER = "big"
GT_MASK_BITORDER = "little"
...
def verify_protocol_and_p06a() -> Dict[str, Any]:
    # Implementation lineage: these exact scripts produced / evaluated the
    # frozen assets. This is checked before any GT recount.
    for label, path, expected in [
        ("FINALB1_SCRIPT", FINALB1_SCRIPT, EXPECTED_FINALB1_SCRIPT_SHA256),
        ("FINALB3_SCRIPT", FINALB3_SCRIPT, EXPECTED_FINALB3_SCRIPT_SHA256),
        ("CM6_SCRIPT", CM6_SCRIPT, EXPECTED_CM6_SCRIPT_SHA256),
    ]:
        if not path.is_file():
            raise FileNotFoundError(path)
        got_code = sha256_file(path)
        if got_code != expected:
            raise RuntimeError(
                f"{label} SHA mismatch expected={expected} observed={got_code}"
            )

    # Static serialization-contract sentinels. No model import/execution.
    b1_text = FINALB1_SCRIPT.read_text(encoding="utf-8", errors="strict")
    cm6_text = CM6_SCRIPT.read_text(encoding="utf-8", errors="strict")
    b3_text = FINALB3_SCRIPT.read_text(encoding="utf-8", errors="strict")
    if 'BITORDER = "big"' not in b1_text:
        raise RuntimeError("FinalB1 BITORDER='big' contract changed.")
    if 'bitorder="little"' not in cm6_text:
        raise RuntimeError("CM6 little-bitorder GT serialization contract changed.")
    if "CM6_GT_PACKED" not in b3_text or "B1_DIR" not in b3_text:
        raise RuntimeError("FinalB3 B1/CM6 outcome lineage contract changed.")
    ...
    return {
        ...
        "producer_code": {
            "FinalB1": {
                "path": str(FINALB1_SCRIPT),
                "sha256": EXPECTED_FINALB1_SCRIPT_SHA256,
                "prediction_mask_bitorder": PRED_MASK_BITORDER,
            },
            "FinalB3": {
                "path": str(FINALB3_SCRIPT),
                "sha256": EXPECTED_FINALB3_SCRIPT_SHA256,
            },
            "CM6": {
                "path": str(CM6_SCRIPT),
                "sha256": EXPECTED_CM6_SCRIPT_SHA256,
                "gt_mask_bitorder": GT_MASK_BITORDER,
            },
        },
    }
...
def packed_counts(
    ...
    """
    Pixelwise TP/FP/FN using each asset's *producer* serialization convention.

    FinalB1 prediction masks: np.packbits(..., bitorder="big")
    CM6 GT masks          : np.packbits(..., bitorder="little")

    The old v1 implementation directly ANDed packed bytes, which is invalid
    when the two arrays use different bit order. This fix decodes both to the
    same physical pixel order before counting.
    """
    ...
    for start in range(0, N_SLICES, batch):
        ...
        pp = np.asarray(pred[start:stop], dtype=np.uint8)
        gg = np.asarray(gt[start:stop], dtype=np.uint8)

        p = np.unpackbits(
            pp,
            axis=1,
            count=SEG_SIZE * SEG_SIZE,
            bitorder=PRED_MASK_BITORDER,
        ...
        g = np.unpackbits(
            ...
            axis=1,
            count=SEG_SIZE * SEG_SIZE,
            bitorder=GT_MASK_BITORDER,
        ).astype(bool, copy=False)

        tp[start:stop] = np.logical_and(p, g).sum(axis=1, dtype=np.int64)
        fp[start:stop] = np.logical_and(p, ~g).sum(axis=1, dtype=np.int64)
        fn[start:stop] = np.logical_and(~p, g).sum(axis=1, dtype=np.int64)
...
def policy_metrics(
    ...
    adapt_arr = np.asarray(adapt, dtype=bool)
    if row_indices is not None and len(adapt_arr) == len(idx):
        # Occurrence-local policy mask (used for matched random inside a
        # patient-cluster bootstrap where duplicated patients must remain
        # independent bootstrap occurrences).
        a = adapt_arr
    else:
        a = adapt_arr[idx]
...
def random_exact_cell_selection_local(
    d: pd.DataFrame,
    ...
    """
    Matched-random policy on bootstrap *occurrences*, not original global rows.

    This is required because a patient-cluster bootstrap can sample the same
    physical patient more than once. A global boolean mask would collapse
    duplicate occurrences and invalidate exact matched coverage.
    """
    idx = np.asarray(row_indices, dtype=np.int64)
    cell = d["cell_i"].to_numpy(dtype=np.int64)[idx]
    sel_local = np.zeros(len(idx), dtype=bool)

    for c in range(N_CELLS):
        local_pos = np.flatnonzero(cell == c)
        k = int(k_by_cell[c])
        if k < 0 or k > len(local_pos):
            raise RuntimeError(
                f"Local random k invalid cell={c} k={k} n={len(local_pos)}"
            )
        if k > 0:
            chosen_local = rng.choice(local_pos, size=k, replace=False)
            sel_local[chosen_local] = True
...
def paired_patient_bootstrap(
    ...
    for rep in tqdm(
        ...
        for _ in range(BOOTSTRAP_INNER_RANDOM_DRAWS):
            rsel_local = random_exact_cell_selection_local(
                d, idx, k_by_cell, rng
            )
...
def self_test():
    # Same physical 8 pixels serialized with opposite bitorders.
    physical = np.asarray([[1,0,1,1,0,0,0,0]], dtype=np.uint8)
    pb = np.packbits(physical, axis=1, bitorder="big")
    gl = np.packbits(physical, axis=1, bitorder="little")
    p = np.unpackbits(pb, axis=1, count=8, bitorder=PRED_MASK_BITORDER)
    g = np.unpackbits(gl, axis=1, count=8, bitorder=GT_MASK_BITORDER)
    assert np.array_equal(p, physical)
    assert np.array_equal(g, physical)
    assert np.array_equal(p, g)

    # Demonstrate why direct packed-byte equality/AND is not a valid parity test.
    assert not np.array_equal(pb, gl)

    assert PACKED_BYTES == 15488
    assert N_ROWS == 8262
    assert int(math.floor(N_SLICES * 0.5)) == 688
    print("SELF_TEST_DUAL_BITORDER_DECODE=PASS")
...
def main():
    ...
    ap.add_argument(
        ...
        action="store_true",
        help=(
            "Recount frozen FinalB1 masks using producer-aware bitorders and "
            "verify exact FinalB3 slice-Dice/HARM parity, then stop before "
            "any utility/randomization/bootstrap metrics."
        ),
    )
    args = ap.parse_args()
    ...
    print("PRED bitorder   =", PRED_MASK_BITORDER)
    print("GT bitorder     =", GT_MASK_BITORDER)
    print("LINEAGE=PASS")
    ...
    if OUT_DIR.exists():
        raise FileExistsError(
            f"Never overwrite P06B fix1 output: {OUT_DIR}\n"
            "Use _fix2 only for implementation repair."
        )
