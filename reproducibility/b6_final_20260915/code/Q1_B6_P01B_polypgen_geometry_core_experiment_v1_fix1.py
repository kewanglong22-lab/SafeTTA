...
Fix1 repairs historical outcome-table schema/path binding only.\n\nThis is the first scientific P01B experiment after:
...
VERSION = "2026-09-15-B6-P01B-GEOM-CORE-v1-fix1"
...
OUT_DIR = ROOT / "B6_P01B_polypgen_geometry_core_experiment_v1_fix1"
...
DELTA_ALIASES = [
    ...
    "candidate_minus_source_dice", "deltaDice",
]
SCORE_ALIASES = [
...
def standardize_outcome_table(path: Path, df: pd.DataFrame) -> List[Tuple[str, pd.DataFrame, Dict[str, Any]]]:
    ...
    Fix1 changes only schema binding:
    - case-insensitive aliases;
    - broader historical aliases;
    - frozen HARM can be reproduced from delta_dice <= -0.02 when a stored
      harm column is absent;
    - action may come from a stored action/method column or, if absent, the
      frozen artifact path.

    No scientific threshold or support definition is changed.
    """
    ...
    if hc is not None:
        ...
        if harm_num.isna().any():
            # Historical string labels are allowed only when they map exactly.
    ...
    if ac is not None:
        ...
        action_source = ac
        # Some historical action columns contain generic labels while the
        # artifact path carries the concrete frozen action.
    ...
    for action in sorted(set(actions) - {""}):
        ...
        sub = base.loc[mask].copy().reset_index(drop=True)
        if len(sub) == 0:
            continue
...
def choose_source_outcomes(inv: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    if len(diag):
        show_cols = [
            "action", "rows", "samples", "states", "unique_keys",
...
def choose_target_outcomes(inv: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    if len(diag):
        show_cols = [
            "rows", "samples", "states", "unique_keys",
            "exact_expected_panel", "selection_score",
            "harm_source", "action_source", "path",
        ]
        print("\nTARGET MEMO outcome binding candidates (top 20):")
        print(
            diag.sort_values(
                ["exact_expected_panel", "selection_score", "rows"],
                ascending=[False, False, False],
    ...
    exacts = [
        ...
        if x[1]["exact_expected_panel"]
    ]
    if not exacts:
        raise RuntimeError(
            "TARGET_OUTCOME_BINDING_FAILED_FIX1: no exact "
            f"{TARGET_ROWS}-row/{TARGET_IMAGES}-image/{TARGET_STATES}-state "
            "MEMO outcome candidate."
        )
    ...
    for other_sub, other_meta in exacts[1:]:
        oth = other_sub.sort_values(KEY_COLS).reset_index(drop=True)
        if not ref[KEY_COLS].equals(oth[KEY_COLS]):
            continue
        if not np.array_equal(
            ...
            raise RuntimeError(
                "TARGET_OUTCOME_BINDING_AMBIGUOUS_FIX1: exact MEMO candidates "
                "with identical keys disagree on HARM labels."
            )
