...
Fix5 retains the R38A1A historical NeoPolyp mask adjudication and repairs only PolypGen R33A2B packed-mask row binding: the exact pre-HARM memo_changed_pixels vector is used as an outcome-free row-order sentinel for each 1532-row state NPZ.\n\nThis is the first scientific P01B experiment after:
...
VERSION = "2026-09-15-B6-P01B-GEOM-CORE-v1-fix5"
...
OUT_DIR = ROOT / "B6_P01B_polypgen_geometry_core_experiment_v1_fix5"
...
def packed_changed_pixel_count(
    source_packed: np.ndarray,
    candidate_packed: np.ndarray,
) -> np.ndarray:
    """
    Exact binary-mask changed-pixel count from packed bytes.

    352*352 = 123904 bits = 15488 bytes exactly, so there are no padding bits.
    """
    a = np.asarray(source_packed, dtype=np.uint8)
    b = np.asarray(candidate_packed, dtype=np.uint8)
    if a.shape != b.shape:
        raise RuntimeError(
            f"Packed mask shape mismatch: {a.shape} vs {b.shape}"
        )
    if a.ndim != 2 or a.shape[1] != PACKED_BYTES:
        raise RuntimeError(
            f"Unexpected packed mask shape={a.shape}"
        )
    ...
    return lut[xor].sum(axis=1, dtype=np.int64)


def resolve_r33_pre_harm_row_binding(
    npz_path: Path,
    expected_state_id: str,
    required_state_keys: pd.DataFrame,
    source_packed: np.ndarray,
    memo_packed: np.ndarray,
) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """
    Fix5: exact outcome-free row-order binding for the R33A2B state NPZ.

    The local NPZ does not carry a direct sample-id manifest. However the
    *pre-HARM* R33A2B frozen score lock contains, for every row:
      sample_id, model_state_id, action, memo_changed_pixels.

    memo_changed_pixels is generated from the SOURCE/MEMO candidate masks
    before HARM reveal. We therefore use it only as a provenance sentinel:

      packed SOURCE/MEMO row i -> changed-pixel count
      must equal frozen pre-HARM memo_changed_pixels row i

    for all 1532 rows of the state.

    This establishes exact row order without using Dice, DeltaDice, HARM,
    BENEFIT, AUROC, AUPRC, or any target fitting.
    """
    p = R33A2B_SCORE_PRE_HARM
    if not p.is_file():
        raise FileNotFoundError(p)

    score = pd.read_csv(p, low_memory=False)
    required_cols = {
        "sample_id",
        "model_state_id",
        "action",
    ...
    missing = sorted(required_cols - set(score.columns))
    if missing:
        raise RuntimeError(
            f"R33A2B pre-HARM score lock missing columns: {missing}"
        )

    sub = score[
        score["model_state_id"].astype(str).str.strip()
        == expected_state_id
    ].copy()

    if len(sub) != TARGET_IMAGES:
        raise RuntimeError(
            f"{expected_state_id}: pre-HARM rows={len(sub)} "
            f"!= {TARGET_IMAGES}"
        )
    if sub["sample_id"].astype(str).nunique() != TARGET_IMAGES:
        raise RuntimeError(
            f"{expected_state_id}: pre-HARM sample_id not unique."
        )
    if set(sub["action"].map(normalize_action_value)) != {"MEMO"}:
        raise RuntimeError(
            f"{expected_state_id}: pre-HARM action changed."
        )

    frozen_changed = pd.to_numeric(
        sub["memo_changed_pixels"],
        errors="raise",
    ).to_numpy(dtype=np.int64)

    computed_changed = packed_changed_pixel_count(
        source_packed,
        memo_packed,
    )
    if computed_changed.shape != (TARGET_IMAGES,):
        raise RuntimeError(
            f"{expected_state_id}: changed-pixel vector shape="
            f"{computed_changed.shape}"
        )

    mismatch = computed_changed != frozen_changed
    mismatch_rows = int(mismatch.sum())
    max_abs = int(
        np.max(np.abs(computed_changed - frozen_changed))
    ) if len(computed_changed) else 0

    if mismatch_rows != 0:
        raise RuntimeError(
            f"{npz_path}: pre-HARM changed-pixel sentinel mismatch for "
            f"{expected_state_id}: mismatch_rows={mismatch_rows}/"
            f"{TARGET_IMAGES}, max_abs_diff={max_abs}. "
            "Do not infer row order."
        )

    mapping = pd.DataFrame({
        "sample_id": norm_id(sub["sample_id"]),
        "model_state_id": expected_state_id,
        "_mask_row": np.arange(TARGET_IMAGES, dtype=np.int64),
    })
    ...
    if merged["_mask_row"].isna().any():
        missing_rows = int(merged["_mask_row"].isna().sum())
        raise RuntimeError(
            f"{expected_state_id}: pre-HARM row binding misses "
            f"{missing_rows}/{len(req)} frozen target keys."
        )

    if len(merged) != TARGET_IMAGES:
        raise RuntimeError(
            f"{expected_state_id}: bound rows={len(merged)} "
            f"!= {TARGET_IMAGES}"
        )

    return merged, {
        "binding_source": "R33A2B pre-HARM score lock file order",
        "score_lock_path": str(p),
        "score_lock_sha256": sha256_file(p),
        "sentinel": "memo_changed_pixels",
        "sentinel_rows": TARGET_IMAGES,
        "sentinel_mismatch_rows": mismatch_rows,
        "sentinel_max_abs_diff": max_abs,
...
def bind_r33_polypgen_memo_geometry(
    ...
    for idx in range(3):
        ...
        bound, row_binding_meta = resolve_r33_pre_harm_row_binding(
            npz_path=npz_path,
            expected_state_id=state_id,
            required_state_keys=state_req,
            source_packed=z["source_masks_packed"],
            memo_packed=z["memo_masks_packed"],
        )
        rr = bound["_mask_row"].to_numpy(dtype=np.int64)
...
def main():
    ...
    print("MEMO_ROW_BINDING_SENTINEL=memo_changed_pixels EXACT_1532_PER_STATE")
