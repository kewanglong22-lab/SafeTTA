...
VERSION = "2026-09-15-B6-P07A-v1-fix1"
...
OUT_DIR = ROOT / "B6_P07A_harm_margin_sensitivity_asset_binding_audit_v1_fix1"
...
def first_alias(cols: List[str], aliases: List[str]) -> str | None:
    """
    Exact alias match first, then conservative semantic-name matching.

    P07A v1 used only exact aliases, which missed frozen R33 columns whose
    names carry suffixes such as *_PRE_HARM_LOCK or action-specific prefixes.
    This fix changes only schema discovery; it does not read outcome values.
    """
    ...
    # 2) exact alias as a substring, preferring the shortest column name
    substring_hits = []
    for c in cols:
        lc = c.lower()
        for a in aliases:
            ...
            if la in lc:
                ...
                break
    if substring_hits:
        return sorted(substring_hits, key=lambda x: (len(x), x.lower()))[0]

    return None


def detect_score_col(cols: List[str]) -> str | None:
    exact = first_alias(cols, SCORE_ALIASES)
    ...
    for c in cols:
        lc = c.lower()
        if (
            ("score" in lc or "risk" in lc or "prob" in lc)
            and any(tok in lc for tok in ["harm", "safe", "transition", "memo", "external"])
        ):
            # Avoid obvious labels/metrics.
            if not any(tok in lc for tok in ["auroc", "auprc", "label", "harmful"]):
                hits.append(c)

    return sorted(hits, key=lambda x: (len(x), x.lower()))[0] if hits else None


def detect_delta_col(cols: List[str]) -> str | None:
    exact = first_alias(cols, DELTA_ALIASES)
    if exact is not None:
        return exact
    ...
    for c in cols:
        lc = c.lower()
        if (
            ("delta" in lc and "dice" in lc)
            or ("dice" in lc and "minus" in lc and "source" in lc)
            or ("dice_change" in lc)
        ):
            hits.append(c)
...
def inspect_csv(path: Path) -> Dict[str, Any] | None:
    ...
    score = detect_score_col(cols)
    delta = detect_delta_col(cols)
...
def main():
    ...
    if args.self_test:
        cols = [
            "sample_id",
            "model_state_id",
            "external_safettta_score_pre_harm_lock",
            "memo_delta_dice_minus_source",
        ]
        assert detect_score_col(cols) == "external_safettta_score_pre_harm_lock"
        assert detect_delta_col(cols) == "memo_delta_dice_minus_source"
