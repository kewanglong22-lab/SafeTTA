...
The audit binds:
1) canonical TENT1 and PL-CONF90 action scripts;
...
OUT_DIR = ROOT / "B6_P05_transition_runtime_asset_binding_audit_v1"
...
def canonical_checks() -> Dict[str, Any]:
    ...
    if "run_deeplab_state" not in pl["functions"]:
        raise RuntimeError("Canonical PL script lacks run_deeplab_state.")
    ...
    pl_required_any = [
        "categorical_pseudolabel",
        "PL",
        "pseudo",
    ]
    if not any(x in pl["text"] for x in pl_required_any):
        raise RuntimeError("Canonical PL execution anchors incomplete.")

    return {
        "tent": {
            "path": str(TENT_SCRIPT),
            "sha256": sha256_file(TENT_SCRIPT),
            "run_deeplab_state": True,
        },
        "pl_conf90": {
            "path": str(PL_SCRIPT),
            "sha256": sha256_file(PL_SCRIPT),
            "run_deeplab_state": True,
        },
        "execution_context": {
