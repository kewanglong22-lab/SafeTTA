...
try {
    ...
    & git add -- experiments/R31_action_transfer experiments/R31_controller_negative experiments/R32_validation experiments/R33_external_joint_shift provenance/R31_R33/sha_bound_ancestors
    ...
    if ($staged.Count -eq 0) { throw "No staged changes detected." }
    Write-Host "Staged files:"
    $staged | ForEach-Object { Write-Host "  $_" }

    & git commit -m "Add exact retained R31-R33 experiment scripts"
    if ($LASTEXITCODE -ne 0) {
        throw "git commit failed. If Git reports missing user.name/user.email, configure your Git identity and rerun."
    }
