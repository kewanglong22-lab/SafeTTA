...
EXPECTED_R13A_FIX1_LOCK_SHA = "2db101b8fd4c7c15c007b044752c5621a82a35eed85d25e6f7c7b572b8a8fe27"
EXPECTED_R13A_FIX1_SCRIPT_SHA = "459832b23495b276279a879c2993fa91aea52a3c36e61a525be6b8d255022f83"

OUTPUT_DIR = OUT/"Q1_R13A_confident_pseudolabel_three_family_feasibility_fix2_v1"
...
PASS_DECISION = "PL_CONF90_THREE_FAMILY_EPISODIC_RESET_CORRECTED_NONDEGENERACY_PASS"
STOP_DECISION = "PL_CONF90_THREE_FAMILY_EPISODIC_RESET_CORRECTED_NONDEGENERACY_STOP"
...
def snapshot_buffers(model):
    """
    Snapshot all persistent/non-persistent model buffers by object reference.
    This is required for true episodic reset because BatchNorm running_mean,
    running_var and num_batches_tracked can mutate during adaptation even when
    trainable parameters are restored.
    """
    return [(b, b.detach().clone()) for b in model.buffers()]


def restore_buffers(buffer_values):
    import torch
    with torch.no_grad():
        for b, v in buffer_values:
            b.copy_(v)


def max_buffer_delta(buffer_values):
    vals = [
        float((b.detach().to(dtype=v.dtype) - v).abs().max().cpu())
        for b, v in buffer_values
        if b.numel() > 0
    ]
    return max(vals) if vals else 0.0
...
def validate_lineage():
    for p in (R05D3, R12D_LOCK, R13A_FIX1_LOCK):
        if not p.exists():
    ...
    if sha256_file(R13A_FIX1_LOCK) != EXPECTED_R13A_FIX1_LOCK_SHA:
        raise RuntimeError("R13A Fix1 lock SHA mismatch.")
    fix1 = json.loads(R13A_FIX1_LOCK.read_text(encoding="utf-8"))
    if fix1.get("decision") != "PL_CONF90_THREE_FAMILY_TECHNICAL_NONDEGENERACY_PASS":
        raise RuntimeError("R13A Fix1 did not PASS.")

    return {
        "r05d3_script_sha256": sha256_file(R05D3),
        "r12d_stop_lock_sha256": sha256_file(R12D_LOCK),
        "r13a_fix1_lock_sha256": sha256_file(R13A_FIX1_LOCK),
        "r13a_fix1_script_sha256_expected": EXPECTED_R13A_FIX1_SCRIPT_SHA,
        "fix_reason": "Fix1 restored trainable parameters but not mutable model buffers; full episodic reset requires both.",
    }
...
def common_row(state, target, idx, parity, confident, total, loss, delta, changed, updated, buffer_delta_after_adapt):
    return {
        "probe_index": idx,
        ...
        "buffer_max_abs_delta_after_adaptation": float(buffer_delta_after_adapt),
    }
...
def run_binary_family(r05, state, targets, device, spack):
    ...
    source_buffers = snapshot_buffers(model)
    rows = []
    ...
    for idx in tqdm(
        ...
        restore_buffers(source_buffers)
        source_mode()
        with torch.no_grad():
        ...
        rows.append(
            common_row(
                ...
                updated,
                max_buffer_delta(source_buffers),
            )
        )
...
def self_test():
    ...
    class _BufToy(torch.nn.Module):
        def __init__(self):
            super().__init__()
            self.register_buffer("running", torch.tensor([1.0]))
    toy = _BufToy()
    bv = snapshot_buffers(toy)
    toy.running.add_(3.0)
    assert max_buffer_delta(bv) == 3.0
    restore_buffers(bv)
    assert max_buffer_delta(bv) == 0.0

    print("BUFFER_EPISODIC_RESET_TEST_PASS")
...
def run(args):
    print("===== R13A CONFIDENT PSEUDOLABEL THREE-FAMILY FEASIBILITY FIX2 =====")
    print("action=", ACTION)
    ...
    print("episodic reset implementation=trainable parameters + ALL mutable buffers")
    print("Fix1 full-run parity failure diagnosis=buffer state was not reset")
    ...
    state_summary = (
        ...
        .agg(
            ...
            buffer_delta_after_adapt_max=("buffer_max_abs_delta_after_adaptation","max"),
        )
    )
    ...
    lock = {
        ...
        "action_semantics": {
            ...
            "episodic_reset_components": ["trainable_parameters", "all_model_buffers"],
        ...
        "checks": {
            ...
            "full_buffer_reset_implemented": True,
        },
