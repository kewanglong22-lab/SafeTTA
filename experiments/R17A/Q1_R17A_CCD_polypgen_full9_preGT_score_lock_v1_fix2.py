...
FIX2 SCIENTIFIC CONTRACT
------------------------
The historical Fix1 soft logits no longer exist, so exact historical soft
probabilities cannot be reconstructed from hard masks alone.

fix2 therefore generates NEW SicTTA-CCD baseline scores from:
  - the exact SHA-locked SOURCE checkpoints,
  - the exact SHA-locked PolypGen images,
  - the exact historical SegFormer preprocessing/model code,
  - the exact package/HF processor environment frozen in the public release.

The historical hard-mask locks are used only as a DIAGNOSTIC provenance check.
Any mismatch is recorded exactly and is NOT tuned away. The runner does not
claim bit-exact reconstruction of historical soft logits.

This is valid for R17A because the baseline itself is a new pre-adaptation
score generated before any target-GT/HARM join.
...
FIX1 REPLAY VALIDATION
----------------------
The v1 runner compared recomputed CCD floats with values written to CSV and
then parsed back as float64. Decimal CSV round-tripping can change only the
binary floating representation at approximately 1e-16, so that is not a valid
bit-identity test of the frozen CSV artifact.

fix1 introduces NO numeric tolerance. It compares the recomputed values with
the exact decimal strings locked in the prior CSV. The prior CSV and its
...
VERSION = "2026-09-09-Q1-R17A-CCD-POLYPGEN-FULL9-PREGT-SCORE-LOCK-v1-fix2"
DEFAULT_ROOT = Path(r"F:\MEDSEG_SAFETTA")
OUT_REL = Path(r"outputs\Q1_R17A_CCD_polypgen_full9_preGT_score_lock_v1_fix2")
...
HF_SNAPSHOT = "80983a413c30d36a39c20203974ae7807835e2b4"
HF_CONFIG_SHA256 = "e2378ac7c7a6981d6bdd1d9ccf29b611264c382afce89c561f39c9291164ff91"
HF_PREPROCESSOR_SHA256 = "d96ab2cb112985c19d5ea32f1d1f2369104fc78b1379f5ce26e33ca8646202a7"
...
def validate_frozen_environment(root: Path) -> dict:
    versions = {}
    for pkg, expected in FROZEN_ENV_PACKAGES.items():
        try:
            actual = importlib.metadata.version(pkg)
        except importlib.metadata.PackageNotFoundError:
            raise RuntimeError(f"Frozen environment package missing: {pkg}")
        versions[pkg] = actual
        if actual != expected:
            raise RuntimeError(
                f"Frozen environment mismatch for {pkg}: "
                f"expected={expected} actual={actual}"
            )

    # Validate HF snapshot metadata without network dependence.
    bases = [
        root / "cache" / "huggingface" / "models--nvidia--mit-b0",
        root / "cache" / "huggingface" / "hub" / "models--nvidia--mit-b0",
    ]
    base = next((b for b in bases if b.exists()), None)
    if base is None:
        raise RuntimeError("Frozen Hugging Face mit-b0 cache not found.")

    ref = base / "refs" / "main"
    if not ref.exists():
        raise RuntimeError(f"HF refs/main missing: {ref}")
    snapshot = ref.read_text(encoding="utf-8", errors="ignore").strip()
    if snapshot != HF_SNAPSHOT:
        raise RuntimeError(
            f"HF mit-b0 snapshot mismatch: expected={HF_SNAPSHOT} actual={snapshot}"
        )

    snap = base / "snapshots" / HF_SNAPSHOT
    config = snap / "config.json"
    preproc = snap / "preprocessor_config.json"
    validate_sha(config, HF_CONFIG_SHA256, "HF mit-b0 config.json")
    validate_sha(preproc, HF_PREPROCESSOR_SHA256, "HF mit-b0 preprocessor_config.json")

    return {
        "packages": versions,
        "hf_cache_base": str(base),
        "hf_snapshot": snapshot,
        "hf_config_sha256": HF_CONFIG_SHA256,
        "hf_preprocessor_sha256": HF_PREPROCESSOR_SHA256,
    }
...
def replay_partial6_and_advance_rng(
    ...
    # IMPORTANT:
    # The frozen 6-state artifact is a CSV. Parsing decimal CSV values back to
    # float64 does not guarantee preservation of the original in-memory binary
    # floating representation. That can create ~1e-16 differences even when
    # the locked serialized decimal value is exactly reproduced.
    #
    # We do NOT use or widen any numeric tolerance. Instead, validate against
    # the exact decimal strings locked in the frozen CSV.
    if "ccd_risk" not in locked6_text.columns:
        raise RuntimeError("Locked six-state text table missing ccd_risk.")

    locked_ccd_text = locked6_text["ccd_risk"].astype(str).tolist()
    replay_ccd_text = [str(float(x)) for x in replay["ccd_risk"].tolist()]

    if len(locked_ccd_text) != len(replay_ccd_text):
        raise RuntimeError(
            f"Partial six-state serialized CCD row mismatch: "
            f"locked={len(locked_ccd_text)} replay={len(replay_ccd_text)}"
        )

    bad_idx = [
        i for i, (replay_txt, locked_txt) in enumerate(
            zip(replay_ccd_text, locked_ccd_text)
        )
        if replay_txt != locked_txt
    ]
    if bad_idx:
        examples = [
            {
                "row": i,
                "replay": replay_ccd_text[i],
                "locked": locked_ccd_text[i],
            }
            for i in bad_idx[:10]
        ]
        raise RuntimeError(
            "Partial six-state CCD serialized-decimal replay mismatch: "
            f"bad={len(bad_idx)} examples={examples}"
        )
    ...
    return {
        ...
        "ccd_replay_validation": "EXACT_LOCKED_CSV_DECIMAL_SERIALIZATION",
...
def infer_one_segformer_state(
    ...
    for i in iterator:
        ...
        # Historical hard-mask parity is diagnostic only in fix2.
        # Do not tune preprocessing or thresholds to force a match.
        frozen_flat = np.unpackbits(frozen, bitorder="big")[: IMAGE_SIZE * IMAGE_SIZE]
        current_flat = np.asarray(sm, dtype=np.uint8).reshape(-1)
        mismatch_pixels = int(np.count_nonzero(current_flat != frozen_flat))
        fg_delta = int(current_fg - frozen_fg)
    ...
    mismatch_samples = int((parity_df["mismatch_pixels"] > 0).sum())
    total_mismatch_pixels = int(parity_df["mismatch_pixels"].sum())
    max_mismatch_pixels = int(parity_df["mismatch_pixels"].max())
    max_abs_fg_delta = int(parity_df["foreground_delta"].abs().max())
    ...
    return score_df, {
        ...
        "rows": len(score_df),
        "historical_hard_mask_exact_matches": int(len(score_df) - mismatch_samples),
        "historical_hard_mask_mismatch_samples": mismatch_samples,
        "historical_hard_mask_total_mismatch_pixels": total_mismatch_pixels,
        "historical_hard_mask_max_mismatch_pixels_per_sample": max_mismatch_pixels,
        "historical_hard_mask_max_abs_foreground_delta": max_abs_fg_delta,
        "historical_hard_mask_parity_role": "DIAGNOSTIC_ONLY_NOT_SCORE_ACCEPTANCE_GATE",
        "parity_df": parity_df,
    }
...
def main() -> int:
    ...
    print("SegFormer historical hard-mask parity: DIAGNOSTIC ONLY")
    print(f"CCD RNG seed: {RNG_SEED}")
    print()

    print("[0/6] Validate frozen package + HF processor environment...")
    frozen_env = validate_frozen_environment(root)
    ...
    # ------------------------------------------------------------------
    print("[1/6] Verify frozen PolypGen image bytes...")
    image_paths = verify_actual_images_once(root, manifest)

    # ------------------------------------------------------------------
    # 4. Re-run first 6 CCD states ONLY to prove lock + advance RNG.
    # ------------------------------------------------------------------
    print("[2/6] Replay first 6 CCD states and continue frozen RNG stream...")
    ...
    print("[3/6] Build exact historical SegFormer context...")
    ...
    print("[4/6] Run 3 SegFormer SOURCE states + diagnostic hard-mask parity...")
    ...
    # ------------------------------------------------------------------
    print("[5/6] Assemble final 9-state PRE-GT score lock...")
    ...
    total_parity_mismatch_samples = int(sum(
        m["historical_hard_mask_mismatch_samples"] for m in parity_meta
    ))
    total_parity_mismatch_pixels = int(sum(
        m["historical_hard_mask_total_mismatch_pixels"] for m in parity_meta
    ))
    total_parity_exact_samples = int(sum(
        m["historical_hard_mask_exact_matches"] for m in parity_meta
    ))

    print("[6/6] Lock provenance + parity limitation disclosure...")

    lock = {
        "status": "PASS",
        "decision": "PASS_R17A_POLYPGEN_CCD_FULL9_CURRENT_ENV_PREGT_SCORE_LOCK_WITH_DISCLOSED_HARD_PARITY_LIMIT",
        "version": VERSION,
        "dataset": "PolypGen",
        ...
        "segformer_contract": {
            ...
            "states": parity_meta,
            "soft_logit_provenance": (
                "NEW_GT_FREE_REGENERATION_IN_FROZEN_RELEASE_ENVIRONMENT_"
                "USING_EXACT_CHECKPOINT_IMAGE_AND_HISTORICAL_CODE"
            ),
            "historical_hard_mask_parity": "DIAGNOSTIC_ONLY",
            "historical_hard_mask_exact_samples": total_parity_exact_samples,
            "historical_hard_mask_mismatch_samples": total_parity_mismatch_samples,
            "historical_hard_mask_total_mismatch_pixels": total_parity_mismatch_pixels,
            "bitexact_historical_soft_logits_claimed": False,
        },
        ...
        "next": "REVIEW_PARITY_AUDIT_THEN_JOIN_ONLY_FROZEN_POLYPGEN_HARM_OUTCOME_AND_EVALUATE_CCD",
    }
    ...
    report_lines = [
        ...
        f"Partial 6-state locked CSV decimal replay exact: {partial_replay['ccd_exact_match']}",
        f"Partial alignment SHA256: {partial_replay['alignment_sha256']}",
        f"SegFormer historical hard-mask exact samples: {total_parity_exact_samples}",
        f"SegFormer historical hard-mask mismatch samples: {total_parity_mismatch_samples}",
        f"SegFormer historical hard-mask total mismatch pixels: {total_parity_mismatch_pixels}",
        "Historical soft-logit bit-exact reconstruction claimed: NO",
        "SegFormer soft logits: NEW GT-free regeneration in frozen release environment",
        ...
        "  Threshold tuning: NO",
        "",
        "GATE=PASS_R17A_POLYPGEN_CCD_FULL9_CURRENT_ENV_PREGT_SCORE_LOCK_WITH_DISCLOSED_HARD_PARITY_LIMIT",
        "NEXT=REVIEW_PARITY_AUDIT_THEN_JOIN_ONLY_FROZEN_POLYPGEN_HARM_OUTCOME_AND_EVALUATE_CCD",
    ]
