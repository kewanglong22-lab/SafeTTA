...
VERSION = "2026-09-09-Q1-R17A-ADIC-MC-DEEPLAB3-PREGT-SCORE-LOCK-v1"
DEFAULT_ROOT = Path(r"F:\MEDSEG_SAFETTA")
OUT_REL = Path(r"outputs\Q1_R17A_ADIC_MC_deeplab3_preGT_score_lock_v1")
...
def compile_exact_input_prefix(r05):
    """
    Extract from the exact SHA-locked run_deeplab_state loop the statements
    up to and including the assignment that creates variable `x`.
    """
    ...
    if candidate_for is None or x_index is None:
        raise RuntimeError(
            "Could not locate top-level x assignment in run_deeplab_state target loop."
        )

    prefix = candidate_for.body[: x_index + 1]

    # Guard: the preprocessing prefix itself must not contain optimizer/backward
    # or GT/outcome semantics.
    prefix_text = "\n".join(
    ...
    if hit:
        raise RuntimeError(
            f"Unsafe/ambiguous DeepLab preprocessing prefix contains {hit}\n"
            f"{prefix_text}"
        )
    ...
    code = compile(
        module,
        filename="<exact_R05_run_deeplab_state_input_prefix>",
        mode="exec",
    )

    return code, prefix_text
