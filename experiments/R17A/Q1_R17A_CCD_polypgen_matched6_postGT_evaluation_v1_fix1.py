...
FIX1
----
The v1 evaluator failed before metric computation because both joined tables
contained a `center` column and pandas produced `center_x/center_y`. fix1
requires exact pre-GT/post-GT center agreement and then restores one canonical
`center` column before the clustered bootstrap.
"""
...
VERSION = "2026-09-09-Q1-R17A-CCD-POLYPGEN-MATCHED6-POSTGT-EVALUATION-v1-fix1"
DEFAULT_ROOT = Path(r"F:\MEDSEG_SAFETTA")
OUT_REL = Path(r"outputs\Q1_R17A_CCD_polypgen_matched6_postGT_evaluation_v1_fix1")
...
def build_matched_panel(ccd: pd.DataFrame, frozen: pd.DataFrame) -> pd.DataFrame:
    ...
    # Both the pre-GT CCD table and the frozen post-GT panel carry `center`.
    # Merge with explicit suffixes, require exact center agreement, then
    # collapse back to one canonical `center` column. The v1 script let pandas
    # create center_x/center_y and later incorrectly requested `center`.
    merged = ccd.merge(
        f[
            keys
            + [
        ...
        how="inner",
        validate="one_to_one",
        suffixes=("_ccd", "_frozen"),
    )

    if "center_ccd" not in merged.columns or "center_frozen" not in merged.columns:
        raise RuntimeError(
            "Expected center_ccd/center_frozen after matched merge, but columns are "
            f"{list(merged.columns)}"
        )

    center_ccd = merged["center_ccd"].astype(str).str.strip()
    center_frozen = merged["center_frozen"].astype(str).str.strip()
    center_mismatch = center_ccd != center_frozen
    if center_mismatch.any():
        ...
        raise RuntimeError(
            "Matched pre-GT CCD vs frozen post-GT center mismatch. "
            f"count={int(center_mismatch.sum())}\n{bad.to_string(index=False)}"
        )

    merged["center"] = center_frozen
    merged = merged.drop(columns=["center_ccd", "center_frozen"])
