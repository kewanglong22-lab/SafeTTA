...
VERSION = "2026-09-12-R32B3-v1-fix1"

ROOT = Path(r"F:\MEDSEG_SAFETTA")
CODE = ROOT / "code"

ORIGINAL_R32B3_SCRIPT = CODE / "Q1_R32B3_matched_three_action_harm_evaluation_v1.py"
EXPECTED_ORIGINAL_R32B3_SCRIPT_SHA256 = (
    "f69308bc895beee3e086b6b067d2c88bfb7fb88931fa6fce8372847af9f7f791"
)
...
R32A1_MECHANISM = R32A1_DIR / "R32A1_FIX1_MECHANISM_READOUT.json"

DEFAULT_OUT = ROOT / "R32B3_matched_three_action_harm_evaluation_v1_fix1"
...
def verify_upstream() -> Dict[str, Any]:
    require_sha(
        ORIGINAL_R32B3_SCRIPT,
        EXPECTED_ORIGINAL_R32B3_SCRIPT_SHA256,
        "original failed R32B3 v1 script",
    )
    ...
    # FIX1: R32A1_FIX1_FINAL_LOCK.json does not contain a primary_model field.
    # The frozen primary representation is stored in the SHA-bound
    # R32A1_FIX1_MECHANISM_READOUT.json.  Verify that chain rather than
    # incorrectly requiring a non-existent final-lock field.
    if not R32A1_MECHANISM.is_file():
        raise FileNotFoundError(R32A1_MECHANISM)

    expected_mechanism_sha = str(a1.get("mechanism_readout_sha256", ""))
    if not expected_mechanism_sha:
        raise RuntimeError(
            "R32A1 final lock lacks mechanism_readout_sha256; "
            "cannot verify frozen primary model."
        )

    observed_mechanism_sha = sha256_file(R32A1_MECHANISM)
    if observed_mechanism_sha != expected_mechanism_sha:
        raise RuntimeError(
            "R32A1 mechanism readout SHA changed. "
            f"expected={expected_mechanism_sha} "
            f"observed={observed_mechanism_sha}"
        )

    mechanism = json.loads(
        R32A1_MECHANISM.read_text(encoding="utf-8")
    )
    if mechanism.get("primary_model") != SAFE_MODEL:
        raise RuntimeError(
            "R32A1 frozen primary model changed: "
            f"{mechanism.get('primary_model')}"
        )
...
def write_analysis_lock(out: Path) -> Path:
    ...
    payload = {
        ...
        "version": VERSION,
        "implementation_fix": (
            "Mechanical metadata validation correction: verify SafeTTA primary "
            "model from SHA-bound R32A1 mechanism readout instead of a "
            "non-existent R32A1 final-lock field."
        ),
        "original_R32B3_script_sha256": EXPECTED_ORIGINAL_R32B3_SCRIPT_SHA256,
...
def main() -> int:
    ...
    print("FIX1                        : SHA-bound R32A1 primary-model metadata validation")
