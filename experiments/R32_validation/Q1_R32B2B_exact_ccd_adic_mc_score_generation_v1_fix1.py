...
VERSION = "2026-09-12-R32B2B-v1-fix1"

EXPECTED_ORIGINAL_R32B2B_SCRIPT_SHA256 = (
    "72a80be846ffe3c8d8348ba9db326734c33f96858efcfeebaaa00cbfad95753a"
)
...
ORIGINAL_R32B2B_SCRIPT = CODE / "Q1_R32B2B_exact_ccd_adic_mc_score_generation_v1.py"
...
DEFAULT_OUT = ROOT / "R32B2B_exact_ccd_adic_mc_score_generation_v1_fix1"
...
def verify_upstream() -> Dict[str, Any]:
    require_sha(
        ORIGINAL_R32B2B_SCRIPT,
        EXPECTED_ORIGINAL_R32B2B_SCRIPT_SHA256,
        "original failed R32B2B v1 script",
    )
...
def discover_state_npz(seed: int) -> Path:
    """
    Resolve the frozen R05D3 DeepLabV3-R50 state NPZ for this seed.

    FIX1 (mechanical only):
    the original v1 fallback glob ``*seed{seed}*.npz`` was too broad because
    PraNet shares training seeds 20260817/18/19.  R32B1 is explicitly locked
    to DeepLabV3-R50, so only DeepLab-prefixed candidates are admissible.

    No score, RNG, model, preprocessing, case subset, or scientific protocol
    ...
    # Exact filename produced by the frozen R05D3 save_state convention.
    exact = R05D3_STATE_DIR / f"deeplabv3_r50_seed{seed}_predictions.npz"
    if exact.is_file():
        candidates.append(exact)

    # Defensive fallback remains family-specific; never admit PraNet or
    # SegFormer files that happen to share the same numeric seed.
    for p in R05D3_STATE_DIR.glob(f"deeplabv3_r50_seed{seed}*.npz"):
        ...
        try:
            with np.load(p, allow_pickle=False) as z:
                ...
                if not required.issubset(set(z.files)):
                    rejected.append((str(p), "missing_required_arrays"))
                    continue

                source_pack = np.asarray(
                ...
                if source_pack.shape[0] != ALL_CASES:
                    rejected.append(
                        (str(p), f"source_rows={source_pack.shape[0]}")
                    )
                    continue
                if a1_pack.shape[0] != ALL_CASES:
                    rejected.append(
                        (str(p), f"a1_rows={a1_pack.shape[0]}")
                    )
                    continue
                if source_pack.shape != a1_pack.shape:
                    rejected.append(
                        (
                            str(p),
                            f"shape_mismatch source={source_pack.shape} "
                            f"a1={a1_pack.shape}",
                        )
                    )
                    continue
            ...
            rejected.append((str(p), f"{type(exc).__name__}: {exc}"))

    if len(valid) != 1:
        raise RuntimeError(
            f"seed={seed}: expected exactly one DeepLabV3-R50 "
            f"1000-case R05D3 state NPZ, "
            f"valid={[str(p) for p in valid]}, rejected={rejected}"
        )

    chosen = valid[0]
    if "deeplabv3_r50" not in chosen.name.lower():
        raise RuntimeError(
            f"seed={seed}: non-DeepLab NPZ escaped family filter: {chosen}"
        )
    return chosen
...
def main() -> int:
    ...
    print("FIX1                          : DeepLab-only R05D3 NPZ resolution")
    ...
    input_lock = {
        ...
        "version": VERSION,
        "implementation_fix": (
            "Mechanical family-specific R05D3 NPZ resolution; "
            "original v1 glob also matched PraNet files sharing DeepLab seeds."
        ),
        "original_R32B2B_script_sha256": EXPECTED_ORIGINAL_R32B2B_SCRIPT_SHA256,
