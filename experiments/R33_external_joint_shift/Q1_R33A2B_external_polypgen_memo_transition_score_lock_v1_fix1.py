...
VERSION = "2026-09-12-R33A2B-v1-fix1"

ROOT = Path(r"F:\MEDSEG_SAFETTA")
CODE = ROOT / "code"

ORIGINAL_R33A2B_SCRIPT = (
    CODE / "Q1_R33A2B_external_polypgen_memo_transition_score_lock_v1.py"
)
EXPECTED_ORIGINAL_R33A2B_SCRIPT_SHA256 = (
    "be72c233dcb4248e9071e8474b8b02c6f025b8fdbc07755401c55e5cd032b048"
)

FAILED_V1_OUT = ROOT / "R33A2B_external_polypgen_memo_transition_score_lock_v1"
...
DEFAULT_OUT = ROOT / "R33A2B_external_polypgen_memo_transition_score_lock_v1_fix1"
...
def verify_upstream() -> Tuple[Dict[str, Any], Dict[str, Any]]:
    require_sha(
        ORIGINAL_R33A2B_SCRIPT,
        EXPECTED_ORIGINAL_R33A2B_SCRIPT_SHA256,
        "original failed R33A2B v1 script",
    )
...
def validate_external_state_cache_source(*, source_out: Path, state_index: int, state: pd.Series):
    npz_path, lock_path = state_cache_paths(source_out, state_index)
    if not npz_path.is_file() or not lock_path.is_file():
        raise FileNotFoundError(f"Missing reusable v1 state cache: {npz_path} / {lock_path}")
    lock = json.loads(lock_path.read_text(encoding="utf-8"))
    if lock.get("status") != "PASS_R33A2B_STATE_SOURCE_MEMO_PRE_HARM_LOCK":
        raise RuntimeError(f"Reusable v1 state lock status drift: {lock.get('status')}")
    if lock.get("model_state_id") != str(state["model_state_id"]):
        raise RuntimeError("Reusable v1 state_id mismatch.")
    if int(lock.get("training_seed")) != int(state["training_seed"]):
        raise RuntimeError("Reusable v1 training_seed mismatch.")
    if lock.get("checkpoint_sha256") != str(state["checkpoint_sha256"]):
        raise RuntimeError("Reusable v1 checkpoint SHA mismatch.")
    if int(lock.get("physical_cases")) != TARGET_CASES:
        raise RuntimeError("Reusable v1 physical-case count mismatch.")
    if lock.get("action") != THIRD_ACTION:
        raise RuntimeError("Reusable v1 action mismatch.")
    if float(lock.get("selected_learning_rate")) != SELECTED_LR:
        raise RuntimeError("Reusable v1 MEMO LR mismatch.")
    if lock.get("prediction_npz_sha256") != sha256_file(npz_path):
        raise RuntimeError("Reusable v1 prediction NPZ SHA mismatch.")
    with np.load(npz_path, allow_pickle=False) as z:
        required = {"source_masks_packed", "memo_masks_packed", "memo_loss_preupdate", "grad_l2", "changed_pixels", "action_seeds"}
        if not required.issubset(set(z.files)):
            raise RuntimeError(f"Reusable v1 NPZ missing arrays={sorted(required-set(z.files))}")
        src = np.asarray(z["source_masks_packed"], dtype=np.uint8)
        memo = np.asarray(z["memo_masks_packed"], dtype=np.uint8)
        if src.shape != (TARGET_CASES, PACKED_BYTES) or memo.shape != src.shape:
            raise RuntimeError(f"Reusable mask shapes SOURCE={src.shape} MEMO={memo.shape}")
        for key in ["memo_loss_preupdate", "grad_l2", "changed_pixels", "action_seeds"]:
            if np.asarray(z[key]).shape != (TARGET_CASES,):
                raise RuntimeError(f"Reusable {key} shape drift.")
    return npz_path, lock_path, lock


def adopt_failed_v1_state_caches(*, out: Path, states: pd.DataFrame) -> List[Dict[str, Any]]:
    rows = []
    if not FAILED_V1_OUT.is_dir():
        return rows
    for sidx, state in states.iterrows():
        src_npz, src_lock, _ = validate_external_state_cache_source(source_out=FAILED_V1_OUT, state_index=int(sidx), state=state)
        dst_npz, dst_lock = state_cache_paths(out, int(sidx))
        if dst_npz.exists() or dst_lock.exists():
            continue
        try:
            os.link(src_npz, dst_npz)
            transfer = "hardlink"
        except Exception:
            shutil.copy2(src_npz, dst_npz)
            transfer = "copy2"
        shutil.copy2(src_lock, dst_lock)
        if sha256_file(dst_npz) != sha256_file(src_npz) or sha256_file(dst_lock) != sha256_file(src_lock):
            raise RuntimeError("Adopted v1 cache SHA changed during transfer.")
        rows.append({
            ...
            "source_lock_sha256": sha256_file(src_lock),
            "destination_lock_sha256": sha256_file(dst_lock),
...
def patch_fix5_state_cardinality(*, r31b2):
    original_import = r31b2.import_module
    fix5_path = Path(r31b2.R30A4B2_FIX5_SCRIPT)
    fix5 = original_import(fix5_path, "r33a2b_fix1_fix5_external_state_cardinality")
    fn = getattr(fix5, "conditioned_pca64_for_one_image", None)
    if fn is None:
        raise RuntimeError("fix5 conditioned_pca64_for_one_image missing.")
    try:
    ...
    for name, value in list(vars(fix5).items()):
        ...
        if isinstance(value, int) and value == 9 and ("STATE" in upper or "MODEL" in upper):
            patched[name] = {"old": int(value), "new": int(TARGET_STATES)}
            setattr(fix5, name, int(TARGET_STATES))
    if not patched:
        raise RuntimeError("No 9-state cardinality global found in fix5; refusing formula changes.")
    def patched_import(path: Path, name: str):
        try:
            same = Path(path).resolve() == fix5_path.resolve()
        except Exception:
            same = str(path) == str(fix5_path)
        if same:
            return fix5
        return original_import(path, name)
    audit = {
        "fix5_path": str(fix5_path),
        "fix5_sha256": sha256_file(fix5_path),
        "conditioned_function": "conditioned_pca64_for_one_image",
        "patched_cardinality_globals": patched,
        "target_states": TARGET_STATES,
        "semantic_formula_replaced": False,
        "source_contains_pooled_region_features": ("pooled_region_features" in fn_source) if fn_source else None,
        "source_contains_pca_transform": (".transform" in fn_source) if fn_source else None,
    }
    return original_import, patched_import, audit
...
def run_exact_semantic_lock(
    *,
    ...
    original_import, patched_import, semantic_patch_audit = patch_fix5_state_cardinality(r31b2=r31b2)
    print("\nR33A2B FIX1 SEMANTIC CARDINALITY ADAPTATION")
    print("  fix5 SHA256     :", semantic_patch_audit["fix5_sha256"])
    print("  patched globals :", semantic_patch_audit["patched_cardinality_globals"])
    print("  formula changed : NO")
    r31b2.import_module = patched_import
    try:
        ...
        r31b2.import_module = original_import
    ...
    return q, dm, table, lock, semantic_patch_audit
...
def main() -> int:
    ...
    print("FIX1                        : 3-state external semantic cardinality guard")
    print("Scientific formula change   : NO")
    ...
    pre_lock_payload = {
        ...
        "version": VERSION,
        "implementation_fix": "Only external 3-state semantic shape/cardinality guard adapted; formulas unchanged.",
        "original_R33A2B_script_sha256": EXPECTED_ORIGINAL_R33A2B_SCRIPT_SHA256,
    ...
    adopted = adopt_failed_v1_state_caches(out=out, states=states)
    adoption_path = out / "R33A2B_FIX1_ADOPTED_V1_STATE_CACHE_AUDIT.csv"
    atomic_csv(pd.DataFrame(adopted), adoption_path)
    if adopted:
        print("\nR33A2B FIX1 V1 STATE-CACHE ADOPTION")
        print("  adopted states :", len(adopted))
        print("  scientific content changed : NO")
    ...
    for sidx, state in states.iterrows():
        npz, lock = run_state_memo(
            ...
            device=device,
            bitorder=bitorder,
            resume=(args.resume or bool(adopted)),
        )
    ...
    # -------------------------------------------------------------------------
    q, dm, semantic_table, semantic_lock, semantic_patch_audit = run_exact_semantic_lock(
        out=out,
        rgb=rgb,
        states=states,
        npz_paths=npz_paths,
    ...
    score_lock = {
        ...
        "implementation_fix1": {
            "semantic_state_cardinality_adaptation": semantic_patch_audit,
            "scientific_formula_changed": False,
            "v1_state_caches_adopted": int(len(adopted)),
            "v1_state_cache_adoption_audit_sha256": sha256_file(adoption_path),
        },
        "representation": {
            "q_source66": (
    ...
    print("Scientific formula change   : NO")
    print("V1 MEMO states reused       :", len(adopted))
    print("Target calibration          : NO")
