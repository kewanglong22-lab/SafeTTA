...
VERSION = "2026-09-05-Q1X-CM5B-v1-fix2"
BUILD = "Q1X_CM5B_UNLABELED_SUPPORT_AWARE_OPERATING_POINT_TRANSPORT_LOCK_FIX2"
...
def load_target_assets(cm5a):
    ...
    required = {
        ...
        "slice_index",
        "risk_score",
        "frozen_source_threshold",
        "fixed_source_gate_adapt",
        "fixed_source_gate_retain_source",
    }
    ...
    # CM5A's actual schema uses `frozen_source_threshold`.
    # Validate that the serialized row-level threshold and gate decisions are
    # exactly consistent with the CM4C frozen final-refit deployment threshold.
    cm4c = load_json(CM4C_LOCK)
    ...
    row_thresholds = target_scores[
        "frozen_source_threshold"
    ].to_numpy(dtype=np.float64)

    if not np.isfinite(row_thresholds).all():
        raise RuntimeError("Nonfinite frozen_source_threshold in CM5A scores.")

    max_threshold_diff = float(
        np.max(np.abs(row_thresholds - expected_threshold))
    )
    if max_threshold_diff > 1e-12:
        raise RuntimeError(
            "CM5A row-level frozen SOURCE threshold differs from CM4C lock: "
            f"max_abs_diff={max_threshold_diff}"
        )

    risk = target_scores["risk_score"].to_numpy(dtype=np.float64)
    adapt_expected = (risk < expected_threshold).astype(np.int64)
    retain_expected = (risk >= expected_threshold).astype(np.int64)

    adapt_got = target_scores[
        "fixed_source_gate_adapt"
    ].to_numpy(dtype=np.int64)
    retain_got = target_scores[
        "fixed_source_gate_retain_source"
    ].to_numpy(dtype=np.int64)

    adapt_mismatch = int(np.count_nonzero(adapt_expected != adapt_got))
    retain_mismatch = int(np.count_nonzero(retain_expected != retain_got))

    if adapt_mismatch != 0 or retain_mismatch != 0:
        raise RuntimeError(
            "CM5A fixed SOURCE gate decisions are inconsistent with the "
            "frozen CM4C threshold: "
            f"adapt_mismatch={adapt_mismatch}, "
            f"retain_mismatch={retain_mismatch}"
        )

    print(
        "CM5A target score schema: PASS "
        "column=frozen_source_threshold"
    )
    print(
        "CM5A frozen SOURCE threshold consistency: PASS "
        f"max_abs_diff={max_threshold_diff:.3e}"
    )
    print(
        "CM5A fixed SOURCE gate decision consistency: PASS "
        "adapt_mismatch=0 retain_mismatch=0"
    )
...
def main():
    ...
    print(
        "===== Q1X CM5B FIX2 UNLABELED SUPPORT-AWARE OPERATING-POINT TRANSPORT ====="
    )
    print(
        "FIX2_CHANGE=CM5A_SCHEMA_FIXED_SOURCE_THRESHOLD_TO_FROZEN_SOURCE_THRESHOLD"
    )
    ...
    lock = {
        ...
        "fix2_schema_correction": {
            "issue": (
                "CM5B Fix1 expected target-score column "
                "`fixed_source_threshold`, while CM5A actually serialized "
                "`frozen_source_threshold`."
            ),
            "correction": (
                "Use the actual CM5A column name and verify exact consistency "
                "with the CM4C frozen final-refit deployment threshold and "
                "both serialized fixed-source gate decisions."
            ),
    ...
    print("Fix2 schema correction only: YES")
