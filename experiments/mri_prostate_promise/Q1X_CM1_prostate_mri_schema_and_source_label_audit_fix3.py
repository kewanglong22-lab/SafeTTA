...
ElementType is audited separately as storage dtype and is NOT required to
match between MRI intensity data and binary/label reference masks.
...
VERSION = "2026-09-04-Q1X-CM1-v1-fix3"
BUILD = "Q1X_CM1_PROSTATE_MRI_SCHEMA_AND_SOURCE_LABEL_AUDIT_FIX3"
...
DEFAULT_OUTPUT = OUT / "Q1X_CM1_prostate_mri_schema_and_source_label_audit_fix3_v1"
...
def audit_promise12_without_gt_decode():
    ...
    image_element_types = Counter()
    reference_element_types = Counter()
    ...
    for cid in sorted(img_by_case):
        ...
        image_element_type = normalize_header_value(
            STORAGE_DTYPE_KEY, ih.get(STORAGE_DTYPE_KEY)
        )
        reference_element_type = normalize_header_value(
            STORAGE_DTYPE_KEY, rh.get(STORAGE_DTYPE_KEY)
        )
        image_element_types[image_element_type] += 1
        reference_element_types[reference_element_type] += 1

        # ElementType is intentionally NOT compared for equality. MRI intensity
        # images and reference masks are expected to use different storage
        # dtypes. Each raw payload is instead checked against its own header.
        ...
        records.append({
            ...
            "image_element_type": image_element_type,
            "reference_element_type": reference_element_type,
            "element_type_equality_required": False,
    ...
    print("image ElementType counts:", dict(image_element_types))
    print("reference ElementType counts:", dict(reference_element_types))
    print("ElementType equality required: NO")
    ...
    return {
        ...
        "image_element_type_counts": dict(image_element_types),
        "reference_element_type_counts": dict(reference_element_types),
        "element_type_equality_required": False,
...
def main():
    ...
    print("FIX3_CHANGE=CORRECT_GRID_COMPATIBILITY_EXCLUDES_STORAGE_DTYPE")
    print("SCIENTIFIC_PROTOCOL_CHANGE=NO")
    print("ELEMENTTYPE_EQUALITY_REQUIRED=NO")
    ...
    lock = {
        ...
        "fix2_reason": (
            "Fix1 required text-identical physical metadata. Fix2 correctly split "
            "physical metadata from grid checks but incorrectly required ElementType "
            "equality. Fix3 defines array-grid compatibility by dimensions/channels "
            "only and audits each image/reference storage dtype against its own RAW "
            "payload without opening GT voxels."
        ),
        "source": {
            "dataset": "Prostate158",
        ...
        "external": {
            ...
            "image_element_type_counts": p12["image_element_type_counts"],
            "reference_element_type_counts": p12["reference_element_type_counts"],
            "element_type_equality_required": False,
    ...
    print("PROMISE12 image ElementType counts:", p12["image_element_type_counts"])
    print("PROMISE12 reference ElementType counts:", p12["reference_element_type_counts"])
    print("PROMISE12 ElementType equality required: NO")
    print(
