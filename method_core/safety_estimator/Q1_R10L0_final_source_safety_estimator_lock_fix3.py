...
def main():
    ...
    print("===== R10L0 FINAL SOURCE SAFETY ESTIMATOR LOCK FIX3 =====")
    ...
    #
    # Fix3 verifies serialization at TWO levels:
    #
    # (A) exact serialized parameter identity:
    #     PCA / imputer / scaler / LR learned arrays must be bitwise identical
    #     after joblib reload.
    #
    # (B) functional deployment-path equivalence:
    #     original transform()->predict_proba() versus
    #     reloaded transform()->predict_proba().
    #
    # BLAS / GEMM execution can produce ~1e-7 level floating-point differences
    # even when serialized parameters are exactly identical, so using a
    # 1e-10 probability tolerance is not a valid corruption test.
    #
    # Frozen functional tolerance:
    #     max absolute probability difference <= 1e-6
    #
    # Additionally, ALL 9000 frozen-threshold binary decisions must be exactly
    # identical between original and reloaded estimators.
    pca_reload = joblib.load(pca_path)
    head_reload = joblib.load(head_path)

    def _assert_array_exact(name, a, b):
        a = np.asarray(a)
        b = np.asarray(b)
        if a.shape != b.shape:
            raise AssertionError(
        ...
        if not np.array_equal(a, b, equal_nan=True):
            ...
            raise AssertionError(
                f"{name}: serialized parameter mismatch; "
    ...
    _assert_array_exact(
        ...
        final_pca.explained_variance_,
        pca_reload.explained_variance_,
    )
    _assert_array_exact(
        "PCA.explained_variance_ratio_",
        final_pca.explained_variance_ratio_,
        pca_reload.explained_variance_ratio_,
    )
    _assert_array_exact(
        "PCA.singular_values_",
        final_pca.singular_values_,
        pca_reload.singular_values_,
    )

    # Exact fitted safety-head state.
    for step_name in ["imputer", "scaler", "lr"]:
        if step_name not in final_head.named_steps:
            raise AssertionError(
                f"Original head missing pipeline step: {step_name}"
            )
        if step_name not in head_reload.named_steps:
            raise AssertionError(
                f"Reloaded head missing pipeline step: {step_name}"
            )

    _assert_array_exact(
        "Imputer.statistics_",
        final_head.named_steps["imputer"].statistics_,
        head_reload.named_steps["imputer"].statistics_,
    )
    _assert_array_exact(
        "Scaler.mean_",
        final_head.named_steps["scaler"].mean_,
        head_reload.named_steps["scaler"].mean_,
    )
    _assert_array_exact(
        "Scaler.scale_",
        final_head.named_steps["scaler"].scale_,
        head_reload.named_steps["scaler"].scale_,
    )
    _assert_array_exact(
        "Scaler.var_",
        final_head.named_steps["scaler"].var_,
        head_reload.named_steps["scaler"].var_,
    )
    _assert_array_exact(
        "LR.coef_",
        final_head.named_steps["lr"].coef_,
        head_reload.named_steps["lr"].coef_,
    )
    _assert_array_exact(
        "LR.intercept_",
        final_head.named_steps["lr"].intercept_,
        head_reload.named_steps["lr"].intercept_,
    )
    _assert_array_exact(
        "LR.classes_",
        final_head.named_steps["lr"].classes_,
        head_reload.named_steps["lr"].classes_,
    )

    # Full 9000-row deployment-path functional comparison.
    z_direct = final_pca.transform(cond)
    X_direct = make_final_input(m2, z_direct)
    p_direct = final_head.predict_proba(X_direct)[:, 1]

    z_reload = pca_reload.transform(cond)
    X_reload = make_final_input(m2, z_reload)
    ...
    # Diagnostic only: fit_transform() vs transform() on the same training
    # cohort. This is NOT a serialization integrity criterion.
    fittransform_vs_transform_max_abs_diff = float(
        np.max(np.abs(p_all - p_direct))
    )

    frozen_functional_tolerance = 1e-6

    direct_decision = (p_direct >= final_threshold)
    reload_decision = (p_reload >= final_threshold)
    threshold_decision_mismatch_count = int(
        np.sum(direct_decision != reload_decision)
    )

    print(
        "Serialized parameter identity:",
        "PASS",
    )
    print(
    ...
    print(
        "Frozen functional tolerance:",
        frozen_functional_tolerance,
    )
    print(
        "Frozen-threshold decision mismatches (9000 rows):",
        threshold_decision_mismatch_count,
    )
    print(
    ...
    if roundtrip_max_abs_diff > frozen_functional_tolerance:
        raise AssertionError(
            "Serialized estimator exceeds frozen functional tolerance."
        )

    if threshold_decision_mismatch_count != 0:
        raise AssertionError(
            "Serialized estimator changes one or more frozen-threshold "
            "decisions."
        )
    ...
    lock_out = {
        ...
        "final_fit": {
            ...
            "external_labels_used": False,
            "serialized_parameter_identity": True,
            "serialized_roundtrip_max_abs_probability_diff": (
                roundtrip_max_abs_diff
            ),
            "serialized_functional_tolerance": (
                frozen_functional_tolerance
            ),
            "frozen_threshold_decision_mismatch_count": (
                threshold_decision_mismatch_count
            ),
