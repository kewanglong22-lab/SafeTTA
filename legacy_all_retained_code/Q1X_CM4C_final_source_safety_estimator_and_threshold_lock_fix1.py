...
VERSION = "2026-09-05-Q1X-CM4C-v1-fix1"
BUILD = "Q1X_CM4C_FINAL_SOURCE_SAFETY_ESTIMATOR_AND_THRESHOLD_LOCK_FIX1"
...
DEFAULT_OUTPUT = (
    OUT
    / "Q1X_CM4C_final_source_safety_estimator_and_threshold_lock_fix1_v1"
)
...
def main():
    ...
    print("===== Q1X CM4C FINAL SOURCE SAFETY ESTIMATOR LOCK =====")
    ...
    if max_reload_diff > 1e-12:
        raise RuntimeError(
            f"Final estimator reload mismatch: {max_reload_diff}"
        )
    ...
    save_json(
        threshold_path,
        {
            ...
            "important_scope_note": (
                "Threshold selected on patient-grouped cross-fitted SOURCE OOF "
                "probabilities; final all-SOURCE refit uses the identical fixed "
                "pipeline and probability semantics. No target information used."
            ),
            "promises12_access": False,
        },
    )
    ...
    lock = {
        ...
        "frozen_source_operating_point": threshold_info,
        "final_estimator": {
            "path": str(model_path),
    ...
    print("\n===== CM4C FINAL =====")
    print("Frozen OOF threshold:", threshold_info["threshold"])
    print(
        "Frozen SOURCE HARM recall:",
        threshold_info["pooled"]["harm_recall"],
    )
    print("Frozen SOURCE FPR:", threshold_info["pooled"]["fpr"])
    print(
        "Frozen SOURCE adaptation coverage:",
