...
VERSION = "2026-08-18-S06-B-v1-fix1"
BUILD = "S06_B_FIX1_SYN45200214_FLAT_RELEASE_LAYOUT"
...
PROTOCOL = (
    ROOT / "docs"
    / "S06_A_polypgen_untouched_external_confirmation_protocol_v1_fix1.md"
)
EXPECTED_PROTOCOL_SHA256 = "d9b02990d2c8f4e1d725b5385dbdda41c4b2f71cf47756d4abe8c100c41f7323"
...
OUTPUT_DIR = (
    ROOT / "outputs" / "S06_B_polypgen_integrity_overlap_audit_v1_fix1"
)
...
def is_paper_style_single_frame_branch(name: str):
    """
    Paper/logical layout:
      Positive/Single_Frames_Centre_Split/data_C1/images_C1/...
    """
    p = re.sub(
        r"[^a-z0-9]+",
        "_",
        normalize_member(name).lower(),
    )
    return (
        "positive" in p
        and "single" in p
        and "frame" in p
        and ("centre" in p or "center" in p)
        and "split" in p
    )


def is_syn45200214_flat_release_member(name: str):
    """
    Official syn45200214 flattened release layout, e.g.
      .../dataC1/imagesC1/foo.jpg
      .../dataC1/masksC1/foo.jpg
    """
    p = normalize_member(name).lower()
    return bool(
        re.search(
            r"(?:^|/)datac([1-6])/(?:images|masks)c\1(?:/|$)",
            p,
        )
    )
...
def detect_center(name: str):
    ...
    patterns = (
        # official syn45200214 flat layout
        r"(?:^|/)datac([1-6])(?:/|$)",
        r"(?:^|/)imagesc([1-6])(?:/|$)",
        r"(?:^|/)masksc([1-6])(?:/|$)",
        # paper/logical layout variants
        r"(?:^|/|_|\-)data[_\-]?c([1-6])(?:/|_|\-|$)",
        r"(?:^|/|_|\-)images?[_\-]?c([1-6])(?:/|_|\-|$)",
        r"(?:^|/|_|\-)masks?[_\-]?c([1-6])(?:/|_|\-|$)",
        r"(?:^|/|_|\-)c([1-6])(?:/|_|\-|$)",
    )
...
def classify_single_frame_member(name: str):
    ...
    paper_layout = is_paper_style_single_frame_branch(n)
    flat_layout = is_syn45200214_flat_release_member(n)

    if not (paper_layout or flat_layout):
        return None, None
    ...
    parts = [x.lower() for x in n.split("/")]

    # Annotation visualizations are never raw inputs.
    if any(
        "bbox" in part or "bounding" in part
        for part in parts
    ):
        return None, None
    ...
    # Exact official release folders first.
    if f"masksc{cnum}" in parts:
        return "mask", center
    if f"imagesc{cnum}" in parts:
        return "image", center

    # Paper/logical folder variants.
    if any("mask" in part for part in parts):
        return "mask", center
    if any(part.startswith("image") for part in parts):
        return "image", center
...
def detected_release_layout(name: str):
    if is_syn45200214_flat_release_member(name):
        return "SYN45200214_FLAT_DATAC_IMAGESC_MASKSC"
    if is_paper_style_single_frame_branch(name):
        return "PAPER_STYLE_POSITIVE_SINGLE_FRAMES"
    return ""


def normalized_pair_stem(name: str):
    stem = Path(normalize_member(name)).stem.lower().strip()
    stem = re.sub(
        r"^(image|images|mask|masks)[_\-\s]*",
        "",
        stem,
    )
    return re.sub(r"[_\-\s]+", "_", stem)
...
def self_test():
    image_path = (
        "PolypGen/Positive/"
        "Single_Frames_Centre_Split/"
        "data_C1/images_C1/frame001.jpg"
    )
    mask_path = (
        "PolypGen/Positive/"
        "Single_Frames_Centre_Split/"
        "data_C6/masks_C6/frame001.jpg"
    )
    bbox_path = (
        "PolypGen/Positive/"
        "Single_Frames_Centre_Split/"
        "data_C2/bbox_image_C2/frame001.jpg"
    )

    kind, center = (
        classify_single_frame_member(
            image_path
        )
    )
    assert kind == "image"
    ...
    # FIX1 official syn45200214 flat release layout.
    flat_image = (
        "PolypGen2021_MultiCenterData/"
        "dataC1/imagesC1/frame001.jpg"
    )
    flat_mask = (
        "PolypGen2021_MultiCenterData/"
        "dataC1/masksC1/frame001.jpg"
    )
    flat_bbox = (
        "PolypGen2021_MultiCenterData/"
        "dataC1/bboximageC1/frame001.jpg"
    )

    kind, center = classify_single_frame_member(flat_image)
    assert kind == "image" and center == "C1"
    kind, center = classify_single_frame_member(flat_mask)
    assert kind == "mask" and center == "C1"
    kind, center = classify_single_frame_member(flat_bbox)
    assert kind is None and center is None
    assert (
        detected_release_layout(flat_image)
        == "SYN45200214_FLAT_DATAC_IMAGESC_MASKSC"
    )
    ...
    pairs, ambiguous = pair_members(toy)
    assert len(pairs) == 1
    ...
    print("SYN45200214_FLAT_LAYOUT_TEST_PASS")
    print("MEMBER_CLASSIFICATION_TEST_PASS")
    print("PAIRING_TEST_PASS")
    print("IMAGE_FINGERPRINT_TEST_PASS")
    print("MASK_BYTE_CONFIDENTIALITY_DESIGN_TEST_PASS")
    print("FROZEN_EXPECTED_COUNTS_TEST_PASS")
