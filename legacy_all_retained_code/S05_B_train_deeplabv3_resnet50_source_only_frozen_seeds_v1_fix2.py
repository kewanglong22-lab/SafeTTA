...
VERSION = "2026-08-17-S05-B-v1-fix2"
BUILD = "S05_B_FIX2_VALID_EMPTY_AUGMENTED_MASKS"
...
PROTOCOL = ROOT / "docs" / "S05_A_second_backbone_replication_protocol_v1_fix2.md"
EXPECTED_PROTOCOL_SHA256 = "fa720a21a5f4efecb9e5e776027d8921eb2e28b2aff875c6fdfb350a765207e7"
...
def assert_original_source_mask_nonempty(mask, sample_id):
    """
    Integrity guard applied BEFORE any training augmentation.
    Original source annotations are required to contain foreground.
    """
    arr = np.asarray(mask.convert("L"), dtype=np.float32)
    if float(arr.max()) <= 0:
        raise RuntimeError(
            f"Corrupt/empty ORIGINAL source mask: {sample_id}"
        )


def mask_to_tensor(mask, allow_empty_after_augmentation=False):
    """
    Convert mask to binary tensor.

    Source-val and original source masks remain strict.
    For source-train only, the already-frozen random crop can legitimately
    produce an all-background crop. That post-augmentation target is accepted.
    """
    arr = np.asarray(mask.convert("L"), dtype=np.float32)
    maxv = float(arr.max())

    if maxv <= 0:
        if allow_empty_after_augmentation:
            binary = np.zeros_like(arr, dtype=np.float32)
            return torch.from_numpy(binary[None].copy()), True
        raise RuntimeError("Empty source mask encountered in a strict context.")

    binary = (arr > 0.5 * maxv).astype(np.float32)
    is_empty = bool(binary.max() <= 0)
    return torch.from_numpy(binary[None].copy()), is_empty
...
class SourcePolypDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # FIX2: original annotation integrity remains strict even though a
        # subsequent training crop may legitimately contain no foreground.
        assert_original_source_mask_nonempty(mask, row["sample_id"])
        ...
        mask_tensor, mask_is_empty_aug = mask_to_tensor(
            mask,
            allow_empty_after_augmentation=self.training,
        )

        # Validation is a strict context and must never become empty.
        if (not self.training) and mask_is_empty_aug:
            raise RuntimeError(
                f"Unexpected empty source-val mask: {row['sample_id']}"
            )

        return {
            "image": image_to_tensor(image),
            "mask": mask_tensor,
            "mask_is_empty_aug": bool(mask_is_empty_aug),
...
def train_one_epoch(
    ...
    for batch in pbar:
        ...
        empty_flags = batch["mask_is_empty_aug"]
        if torch.is_tensor(empty_flags):
            empty_aug_mask_samples += int(empty_flags.sum().item())
        else:
            empty_aug_mask_samples += int(sum(bool(x) for x in empty_flags))
        ...
        pbar.set_postfix(
            ...
            emptyAug=empty_aug_mask_samples,
            gpu_mem=(
    ...
    return (
        loss_sum / max(n_samples, 1),
        empty_aug_mask_samples,
    )
...
def train(args):
    ...
    try:
        for epoch in range(1, EPOCHS + 1):
            ...
            train_loss, empty_aug_mask_samples = train_one_epoch(
                model=model,
                loader=train_loader,
                dataset=train_ds,
                batch_sampler=train_batch_sampler,
            ...
            record = {
                ...
                "empty_aug_mask_samples": empty_aug_mask_samples,
            ...
            print(
                ...
                f"emptyAug={empty_aug_mask_samples} "
        ...
        total_empty_aug_mask_samples = int(
            sum(int(r["empty_aug_mask_samples"]) for r in history)
        )
        peak_gpu_gb = (
        ...
        audit = {
            ...
            "training_empty_augmented_masks_allowed": True,
            "original_source_masks_required_nonempty": True,
            "source_val_masks_required_nonempty": True,
            "total_empty_aug_mask_samples": total_empty_aug_mask_samples,
...
Training:
  ...
  training empty masks allowed only after spatial augmentation=True
  original source masks required non-empty=True
  source-val masks required non-empty=True
  total empty augmented mask samples={total_empty_aug_mask_samples}
  persistent_workers=False
  CUBLAS_WORKSPACE_CONFIG={os.environ.get("CUBLAS_WORKSPACE_CONFIG")}
...
def self_test():
    ...
    # FIX2: original/validation masks are strict, but training crop may be empty.
    positive_mask = Image.fromarray(
        np.pad(
    ...
    assert_original_source_mask_nonempty(positive_mask, "toy_positive")
    pos_tensor, pos_empty = mask_to_tensor(
        positive_mask, allow_empty_after_augmentation=False
    )
    assert tuple(pos_tensor.shape) == (1, 16, 16)
    assert not pos_empty

    empty_mask = Image.fromarray(np.zeros((16, 16), dtype=np.uint8))
    empty_tensor, empty_flag = mask_to_tensor(
        empty_mask, allow_empty_after_augmentation=True
    )
    assert empty_flag
    assert float(empty_tensor.sum()) == 0.0

    try:
        mask_to_tensor(
            empty_mask, allow_empty_after_augmentation=False
        )
        raise AssertionError("Strict empty-mask guard failed.")
    except RuntimeError:
        pass

    try:
        assert_original_source_mask_nonempty(empty_mask, "toy_empty")
        raise AssertionError("Original-mask integrity guard failed.")
    except RuntimeError:
        pass
    ...
    print("ORIGINAL_MASK_INTEGRITY_TEST_PASS")
    print("AUGMENTED_EMPTY_MASK_TEST_PASS")
    print("SOURCE_VAL_EMPTY_MASK_GUARD_TEST_PASS")
