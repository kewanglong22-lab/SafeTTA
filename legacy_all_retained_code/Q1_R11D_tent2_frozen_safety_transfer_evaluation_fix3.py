...
OUTPUT_DIR = ROOT / "outputs" / "Q1_R11D_tent2_frozen_safety_transfer_evaluation_fix3_v1"
...
def build_a2_panel(r05d4, d4, gt_masks):
    ...
    for state_id, g in man.groupby("model_state_id", sort=True):
        ...
        npz_path = Path(npzs[0])
        index_path = Path(idxs[0])
        lock_path = Path(locks[0])
...
def run(args):
    print("===== R11D TENT2 FROZEN SAFETY TRANSFER EVALUATION FIX3 =====")
