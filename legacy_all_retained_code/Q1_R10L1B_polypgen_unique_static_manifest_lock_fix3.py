...
def main():
    ...
    print("===== R10L1B POLYPGEN UNIQUE STATIC MANIFEST LOCK FIX3 =====")
    ...
    print("DUPLICATE POLICY: same RGB+same GT -> keep one; same RGB+different GT -> exclude whole conflict group")
    print()
    ...
    for rgb_sha, g in df.groupby("rgb_sha256_recomputed", sort=True):
        ...
        if gt_unique != 1:
            # Same input bytes with different annotations is label ambiguity.
            # Because this audit is still pre-performance, exclude the ENTIRE
            # conflicting group rather than choosing whichever GT looks better.
            # This is a technical annotation-consistency exclusion, not a
            # performance-driven exclusion.
            for idx in g.index:
                r = df.loc[idx]
                exclude_rows.append({
                    "center": r["center"],
                    ...
                    "gt_sha256": r["gt_sha256_recomputed"],
                    "reason": "EXACT_DUPLICATE_RGB_CONFLICTING_GT_EXCLUDE_GROUP",
                    "retained_representative_sample_id": "",
                })
            continue

        # Identical input and identical annotation: retain one deterministic
        # representative, exclude only redundant copies.
        tmp = g.copy()
    ...
    print("Duplicate/conflict rows excluded:", len(exclusions))
    ...
    if len(exclusions):
        conflict_excluded_rows = int(
            (
                exclusions["reason"]
                == "EXACT_DUPLICATE_RGB_CONFLICTING_GT_EXCLUDE_GROUP"
            ).sum()
        )

    redundant_excluded_rows = 0
    if len(exclusions):
        redundant_excluded_rows = int(
            (
                exclusions["reason"]
                == "EXACT_DUPLICATE_RGB_AND_GT_REDUNDANT"
            ).sum()
        )

    print(
        "Conflicting-annotation duplicate rows excluded:",
        conflict_excluded_rows,
    )
    print(
        "Redundant same-RGB/same-GT rows excluded:",
        redundant_excluded_rows,
    )

    if len(unique_df) < 1500:
        raise AssertionError(
            "Unexpectedly large loss during exact duplicate/conflict exclusion."
        )

    # Rename to future external-validation-facing identifiers.
    #
    # Fix3: R10L1 Fix2 already contains an `image_sha256` audit column.
    # Renaming `rgb_sha256_recomputed` to the same name would create duplicate
    # DataFrame column labels. Then unique_df["image_sha256"] returns a
    # DataFrame rather than a Series, causing the later int(nunique()) failure.
    #
    # The recomputed hashes in THIS lock stage are authoritative, so remove
    # any earlier audit hash columns before assigning the locked names.
    prior_hash_columns_removed = []
    for c in ["image_sha256", "gt_sha256"]:
        if c in unique_df.columns:
            unique_df = unique_df.drop(columns=[c])
            prior_hash_columns_removed.append(c)
    ...
    if unique_df.columns.duplicated().any():
        dup_cols = unique_df.columns[
            unique_df.columns.duplicated()
        ].tolist()
        raise AssertionError(
            f"Duplicate output column labels remain after hash lock: {dup_cols}"
        )

    print(
        "Prior hash columns removed before authoritative rename:",
        prior_hash_columns_removed,
    )
    print(
        "Locked image_sha256 column type:",
        type(unique_df["image_sha256"]).__name__,
    )
    print(
        "Locked unique RGB hashes:",
        int(unique_df["image_sha256"].nunique()),
    )
    ...
    summary = {
        ...
        "prior_hash_columns_removed_before_lock": (
            prior_hash_columns_removed
        ),
        "output_duplicate_column_labels": False,
        "exact_duplicate_or_conflict_rows_excluded": len(exclusions),
        "conflicting_annotation_rows_excluded": conflict_excluded_rows,
        "redundant_same_rgb_same_gt_rows_excluded": redundant_excluded_rows,
        "unpaired_files_excluded": len(unpaired),
        "centers": sorted(unique_df["center"].unique().tolist()),
        "neoPolyp_exact_rgb_duplicates": 0,
        "conflicting_gt_policy": "EXCLUDE_ENTIRE_RGB_CONFLICT_GROUP",
