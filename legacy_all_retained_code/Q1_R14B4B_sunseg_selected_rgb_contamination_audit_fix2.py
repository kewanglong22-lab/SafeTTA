...
OUT = (
    ROOT / "outputs"
    / "Q1_R14B4B_sunseg_selected_rgb_contamination_audit_fix2_v1"
)
...
def load_selected_sun(rows, tar_path: Path):
    """
    Read only the 980 frozen selected SUN RGB members from TAR.
    No GT member is accessed.
    """
    member_to_row = {
        r["frame_member"]: r
        for r in rows
    ...
    if len(member_to_row) != len(rows):
        raise RuntimeError(
            "SUN selected frame_member values are not unique."
        )
    ...
    with tarfile.open(tar_path, "r:gz") as tf:
        ...
        for name in tqdm(
            sorted(target_names),
            desc="Read/decode selected SUN RGB",
            unit="frame",
            dynamic_ncols=True,
        ):
            m = members.get(name)
            if m is None:
                raise RuntimeError(
                    f"Frozen SUN member missing: {name}"
                )

            f = tf.extractfile(m)
            if f is None:
                raise RuntimeError(
                    f"Could not read SUN member: {name}"
                )
    ...
    if found != target_names:
        raise RuntimeError(
            "Not all selected SUN RGB members were read."
        )
...
def read_sun_full_arrays_once(
    ...
    """
    Load only requested SUN frames in one TAR pass.
    Used only after soft candidate discovery.
    """
    if not member_names:
        return {}
    ...
    with tarfile.open(tar_path, "r:gz") as tf:
        ...
        for name in tqdm(
            sorted(member_names),
            desc="Load candidate SUN full RGB",
            unit="frame",
            dynamic_ncols=True,
        ):
            m = lookup.get(name)
            if m is None:
                raise RuntimeError(
                    f"Candidate SUN member missing: {name}"
                )
            f = tf.extractfile(m)
            if f is None:
                raise RuntimeError(
...
def main():
    ...
    print("===== Q1 R14B4B FIX2 SUN-SEG SELECTED RGB CONTAMINATION AUDIT =====")
    print("SUPERSEDES_FIX1=YES")
    ...
    descriptor_csv = (
        args.output_dir
        / "R14B4B_FIX2_SUN_SELECTED_RGB_HASHES.csv"
    )
    write_csv(
        descriptor_csv,
    ...
    exact_csv = (
        args.output_dir
        / "R14B4B_FIX2_EXACT_CONTAMINATION_MATCHES.csv"
    )
    write_csv(
        exact_csv,
        exact_matches,
    ...
    near_csv = (
        args.output_dir
        / "R14B4B_FIX2_NEAR_DUPLICATE_CANDIDATES.csv"
    )
    write_csv(
        near_csv,
        near_rows,
    ...
    internal_csv = (
        args.output_dir
        / "R14B4B_FIX2_SUN_INTERNAL_EXACT_DUPLICATES.csv"
    )
    write_csv(
        internal_csv,
        internal_dups,
    ...
    final_manifest = (
        args.output_dir
        / "R14B4B_FIX2_FINAL_CONFIRMATORY_MANIFEST.csv"
    )
    write_csv(
        final_manifest,
        clean_sun_manifest_rows,
    ...
    exclusion_csv = (
        args.output_dir
        / "R14B4B_FIX2_CASE_EXCLUSIONS.csv"
    )
    write_csv(
        exclusion_csv,
        exclusion_rows,
    ...
    root_csv = (
        args.output_dir
        / "R14B4B_FIX2_S01_ROOT_RESOLUTION.csv"
    )
    write_csv(
        root_csv,
    ...
    lock = {
        ...
        "next_stage": (
            ...
            if final_locked
            else "RESOLVE_R14B4B_FIX2_REVIEW_BEFORE_ANY_INFERENCE"
        ),
    }

    lock_path = (
        args.output_dir
        / "R14B4B_FIX2_SUNSEG_CONTAMINATION_AUDIT_LOCK.json"
    )
