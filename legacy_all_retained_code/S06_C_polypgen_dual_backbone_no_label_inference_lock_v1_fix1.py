...
VERSION = "2026-08-18-S06-C-v1-fix1"
BUILD = "S06_C_FIX1_MATCH_S06B_DECODED_RGB_SHA_SEMANTICS"

PROTOCOL = ROOT / "docs" / "S06_C_polypgen_dual_backbone_no_label_lock_protocol_v1_fix1.md"
EXPECTED_PROTOCOL_SHA256 = "b1ae696b0a8eaf4b668ec0f4d26ebc3e40f05cdbce20a94ade92bf6a51556367"
...
OUTPUT_DIR = ROOT / "outputs" / "S06_C_polypgen_dual_backbone_no_label_lock_v1_fix1"
...
def decoded_rgb_sha256(image: Image.Image):
    """
    Exact S06-B FIX3 decoded-RGB fingerprint:
    SHA256(ASCII("RGB|<width>|<height>|") + uint8 RGB pixel bytes).
    """
    rgb = image.convert("RGB")
    width, height = rgb.size
    header = f"RGB|{width}|{height}|".encode("ascii")
    pixels = np.asarray(rgb, dtype=np.uint8).tobytes()
...
def read_external_image(zf: zipfile.ZipFile, row: dict):
    ...
    actual_decoded_sha = decoded_rgb_sha256(image)
    if actual_decoded_sha != row["image_decoded_rgb_sha256"]:
        raise RuntimeError(
            f"Decoded RGB SHA mismatch: {row['external_id']} "
            f"expected={row['image_decoded_rgb_sha256']} "
            f"actual={actual_decoded_sha}"
        )
    return image
...
def self_test():
    ...
    assert accepted == 0
    # Regression test for the exact S06-B decoded-RGB SHA semantics.
    toy_arr = np.zeros((7, 11, 3), dtype=np.uint8)
    toy_arr[:, 5:, 0] = 123
    toy_image = Image.fromarray(toy_arr, mode="RGB")
    expected_toy_sha = hashlib.sha256(
        b"RGB|11|7|" + toy_arr.tobytes()
    ).hexdigest()
    assert decoded_rgb_sha256(toy_image) == expected_toy_sha
    assert bytes_sha256(toy_arr.tobytes()) != expected_toy_sha
    ...
    print("S06B_DECODED_RGB_SHA_SEMANTICS_TEST_PASS")
