...
VERSION = "2026-08-19-S07-C-v1-fix1"
BUILD = "S07_C_FIX1_OOF_CSV_SCHEMA_ENTROPY_DIAGNOSTICS"
...
OUTPUT_DIR = (
    ROOT / "outputs"
    / "S07_C_pranet_grouped_oof_quantile_utility_feasibility_v1_fix1"
)
...
def validate_csv_schema(rows, fields, label: str):
    if not rows:
        raise RuntimeError(f"{label}: cannot validate empty row list.")
    field_set = set(fields)
    missing_from_schema = sorted(
        set().union(*(set(row.keys()) for row in rows)) - field_set
    )
    if missing_from_schema:
        raise RuntimeError(
            f"{label}: row fields missing from CSV schema: {missing_from_schema}"
        )
...
def run(args):
    ...
    validate_csv_schema(
        oof,
        oof_fields,
        "grouped_oof_action_quantile_predictions",
    )
    write_csv(
        oof_path,
        oof,
    ...
    validate_csv_schema(
        decisions,
        decision_fields,
        "grouped_oof_v2_action_selection",
    )
    write_csv(
        decision_path,
        decisions,
...
def self_test():
    ...
    # FIX1 regression: every inherited S07-B diagnostic field carried
    # into an OOF row must be explicitly serializable.
    toy_oof_row = {
        "seed": 20260817,
        "sample_id": "toy",
        "source_group_id": "toy",
        ...
        "base_case_id": "toy::20260817::identity",
        ...
        "neutral": 1,
        "beneficial": 0,
        "tent_step1_entropy_loss": 0.1,
        "tent_step2_entropy_loss": 0.09,
    ...
    print("OOF_CSV_SCHEMA_ENTROPY_DIAGNOSTICS_TEST_PASS")
