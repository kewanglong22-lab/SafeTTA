...
VERSION = "2026-08-17-S00-A1-v1-fix2"
BUILD = "S00_A1_FIX2_PRANET_COMPATIBLE_GRAYSCALE_MASK_AUDIT"
...
def inspect_mask(path: Path):
    """
    PraNet-compatible mask integrity audit.

    The official PraNet loader reads GT with PIL and converts it to mode "L".
    Therefore raw palette indices are not treated as semantic intensities.
    We audit the decoded L-mode mask instead.

    Accepted semantic forms:
      - hard binary {0,1}
      - hard binary {0,255}
      - grayscale/anti-aliased soft mask with values in [0,255]

    We DO NOT rewrite the mask file.

    A mask is considered structurally valid when:
      - it decodes successfully to grayscale;
      - it is non-empty (max > 0);
      - values are finite 8-bit grayscale values.
    """
    with Image.open(path) as mask:
        mask.load()
        native_mode = mask.mode
        size = tuple(mask.size)
        gray = np.asarray(mask.convert("L"), dtype=np.uint8)

    unique = np.unique(gray)
    unique_int = [int(v) for v in unique.tolist()]
    unique_set = set(unique_int)
    ...
    if unique_set.issubset({0, 1}):
        encoding = "hard_0_1"
    elif unique_set.issubset({0, 255}):
        encoding = "hard_0_255"
    else:
        encoding = "grayscale_soft"

    valid = (
        gray.ndim == 2
        and np.isfinite(gray).all()
        and min_value >= 0
        and max_value <= 255
        and max_value > 0
    )

    # Canonical binary foreground only for integrity/hash diagnostics.
    # It does NOT replace or rewrite the stored GT.
    threshold = 0.5 * max_value if max_value > 0 else 0.0
    ...
    if max_value > 1:
        soft_boundary = (gray > 0) & (gray < max_value)
        soft_boundary_fraction = float(soft_boundary.mean())
    else:
        soft_boundary_fraction = 0.0

    return {
        "size": size,
        "native_mode": native_mode,
        "decoded_mode": "L",
        "unique_values": unique_int,
        "unique_count": len(unique_int),
        "min_value": min_value,
        "max_value": max_value,
        "encoding": encoding,
        "valid": bool(valid),
        "foreground_sha256": fg_hash,
        "foreground_fraction": fg_fraction,
        "soft_boundary_fraction": soft_boundary_fraction,
    }
    ...
    try:
        with zipfile.ZipFile(archive, "r") as zf:
            ...
            for split, dataset, pair_key, image_member, mask_member in tqdm(
                ...
                mask_info = inspect_mask(mask_dest)

                if image_size != mask_info["size"]:
                    raise RuntimeError(
                        f"Image/mask dimension mismatch for {split}/{dataset}/{pair_key}: "
                        f"{image_size} vs {mask_info['size']}"
                    )

                if not mask_info["valid"]:
                    raise RuntimeError(
                        f"Invalid/empty grayscale mask for {split}/{dataset}/{pair_key}: "
                        f"mode={mask_info['native_mode']} "
                        f"min={mask_info['min_value']} max={mask_info['max_value']} "
                        f"unique_count={mask_info['unique_count']}"
                    )
                ...
                rows.append({
                    ...
                    "mask_native_mode": mask_info["native_mode"],
                    "mask_decoded_mode": mask_info["decoded_mode"],
                    "mask_encoding": mask_info["encoding"],
                    "mask_unique_count": mask_info["unique_count"],
                    "mask_min_value": mask_info["min_value"],
                    "mask_max_value": mask_info["max_value"],
                    "mask_soft_boundary_fraction": f"{mask_info['soft_boundary_fraction']:.10f}",
                    "mask_semantically_valid": True,
                    "mask_foreground_fraction": f"{mask_info['foreground_fraction']:.10f}",
                    ...
                    "mask_foreground_sha256": mask_info["foreground_sha256"],
                })
        ...
        for (split, dataset), expected in EXPECTED_COUNTS.items():
            ...
            summary_rows.append({
                ...
                "valid_masks": sum(bool(r["mask_semantically_valid"]) for r in group),
                "hard_0_1_masks": sum(r["mask_encoding"] == "hard_0_1" for r in group),
                "hard_0_255_masks": sum(r["mask_encoding"] == "hard_0_255" for r in group),
                "grayscale_soft_masks": sum(r["mask_encoding"] == "grayscale_soft" for r in group),
                "mean_soft_boundary_fraction": (
                    f"{np.mean([float(r['mask_soft_boundary_fraction']) for r in group]):.10f}"
                    if group else ""
                ),
        ...
        manifest_fields = [
            ...
            "mask_native_mode", "mask_decoded_mode", "mask_encoding",
            "mask_unique_count", "mask_min_value", "mask_max_value",
            "mask_soft_boundary_fraction", "mask_semantically_valid",
        ...
        write_csv(
            ...
            [
                ...
                "valid_masks", "hard_0_1_masks", "hard_0_255_masks",
                "grayscale_soft_masks", "mean_soft_boundary_fraction",
                "mean_mask_foreground_fraction"
            ],
        )
        write_csv(
        ...
        audit = {
            ...
            "all_masks_semantically_valid": all(bool(r["mask_semantically_valid"]) for r in rows),
            "mask_encoding_counts": dict(Counter(r["mask_encoding"] for r in rows)),
            "grayscale_soft_mask_count": sum(r["mask_encoding"] == "grayscale_soft" for r in rows),
        ...
        for row in summary_rows:
            summary_lines.append(
                ...
                f"valid masks={row['valid_masks']}, "
                f"hard01={row['hard_0_1_masks']}, "
                f"hard0255={row['hard_0_255_masks']}, "
                f"soft={row['grayscale_soft_masks']}"
            )
        ...
        summary_lines += [
            ...
            "All masks semantically valid after PIL convert(L): YES",
            f"Mask encoding counts: {dict(Counter(r['mask_encoding'] for r in rows))}",
...
def self_test():
    ...
    # PraNet-compatible grayscale mask tests.
    import tempfile
    with tempfile.TemporaryDirectory() as td:
        td = Path(td)

        p01 = td / "mask01.png"
        Image.fromarray(np.array([[0, 1], [1, 0]], dtype=np.uint8)).save(p01)
        i01 = inspect_mask(p01)
        assert i01["valid"] and i01["encoding"] == "hard_0_1"

        p255 = td / "mask255.png"
        Image.fromarray(np.array([[0, 255], [255, 0]], dtype=np.uint8)).save(p255)
        i255 = inspect_mask(p255)
        assert i255["valid"] and i255["encoding"] == "hard_0_255"

        psoft = td / "mask_soft.png"
        Image.fromarray(
            np.array([[0, 1, 7], [128, 248, 255]], dtype=np.uint8)
        ).save(psoft)
        isoft = inspect_mask(psoft)
        assert isoft["valid"]
        assert isoft["encoding"] == "grayscale_soft"
        assert isoft["soft_boundary_fraction"] > 0.0

        pempty = td / "mask_empty.png"
        Image.fromarray(np.zeros((2, 2), dtype=np.uint8)).save(pempty)
        iempty = inspect_mask(pempty)
        assert not iempty["valid"]

    print("MASK_HARD_01_TEST_PASS")
    print("MASK_HARD_0255_TEST_PASS")
    print("MASK_GRAYSCALE_SOFT_ACCEPT_TEST_PASS")
    print("EMPTY_MASK_REJECTION_TEST_PASS")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "S00-A1 fix2 PraNet-compatible exact parser + selective extraction + integrity audit "
