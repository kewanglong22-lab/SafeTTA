...
VERSION = "2026-08-19-Q1-R05B0-v1"
BUILD = "Q1_R05B0_PICCOLO_CONFIRMATORY_TARGET_SELECTION_LOCK"

ROOT = Path(r"F:\MEDSEG_SAFETTA")

PROTOCOL = (
    ROOT / "docs"
    / "Q1_R05B0_piccolo_confirmatory_target_selection_lock_preregistered_protocol_v1.md"
)
EXPECTED_PROTOCOL_SHA256 = "171a07afcf0278c460ac433088a8f03b4fcaffae88085b1c70e6a704a581f9c6"
...
OUTPUT_DIR = (
    ROOT / "outputs"
    / "Q1_R05B0_piccolo_confirmatory_target_selection_lock_v1"
)
...
def token_hits(text: str):
    low = text.lower()
    return sorted(token for token in SEARCH_TOKENS if token in low)
...
def self_test():
    ...
    assert "piccolo" in SEARCH_TOKENS
    assert "pd178" in SEARCH_TOKENS
    assert "4279017" in SEARCH_TOKENS

    assert token_hits("PICCOLO PD178") == ["pd178", "piccolo"]
    assert token_hits("nothing relevant") == []

    fake_self = Path(
        r"F:\MEDSEG_SAFETTA\docs\Q1_R05B0_piccolo_confirmatory_target_selection_lock_preregistered_protocol_v1.md"
    )
    assert is_self_artifact(fake_self)

    print("FROZEN_SELECTION_TEST_PASS")
    print("TOKEN_SCAN_TEST_PASS")
