...
"""
Q1_R10K2B_dinov2_image_mask_LOMO_evaluation_fix1.py
...
DEFAULT_OUTPUT = (
    "F:/MEDSEG_SAFETTA/outputs/"
    "Q1_R10K2B_dinov2_image_mask_LOMO_evaluation_fix1_v1"
)
...
def load_locked_features(
    ...
    with np.load(image_feature_path, allow_pickle=False) as z:
        required = {"sample_id", "cls"}
        if not required.issubset(z.files):
            raise AssertionError(
                f"Image feature file keys={z.files}, required={required}"
            )

        image_sid = np.asarray(z["sample_id"]).astype(str)
    ...
    image_sid_norm = np.asarray(
        [norm_sid_value(x) for x in image_sid]
    )

    if len(set(image_sid_norm.tolist())) != 1000:
        raise AssertionError("Image feature sample IDs not unique.")
    ...
    with np.load(state_feature_path, allow_pickle=False) as z:
        ...
        if not required.issubset(z.files):
            raise AssertionError(
                f"State feature file keys={z.files}, required={required}"
            )

        state_sid = np.asarray(z["sample_id"]).astype(str)
        state_family = np.asarray(z["model_family"]).astype(str)
        state_id = np.asarray(z["model_state_id"]).astype(str)
        fg = np.asarray(
    ...
    if (
        ...
        raise AssertionError(
            "R10K2A state feature explicit key is not unique."
        )
    ...
    # Explicit-key join. No row-order inference.
    joined = morph.merge(
        state_meta[
            [
                "_sid",
    ...
    ).drop(columns=["model_family_repr"])
...
def main():
    ...
    print("===== R10K2B DINOV2 IMAGE-MASK LOMO EVALUATION FIX1 =====")
