...
- uses the frozen CM3 OOF mask as the SOURCE baseline;
- requires the CM4A Fix3 exact batch-8 replay audit to have PASSed with
  zero binary mismatches across all 15 family/fold combinations;
...
VERSION = "2026-09-05-Q1X-CM4A-v1-fix4"
BUILD = "Q1X_CM4A_SOURCE_OOF_TENT1_OUTCOME_ASSET_LOCK_FIX4"
...
CM4A_EXACT_REPLAY_AUDIT = (
    OUT
    / "Q1X_CM4A_source_oof_exact_batch_replay_audit_fix3_v1"
    / "CM4A_EXACT_BATCH_REPLAY_AUDIT.json"
)
EXPECTED_CM4A_EXACT_REPLAY_AUDIT_SHA = (
    "425242b4515d68997b129f12e74d39a7979bc3786c8e922e49b464e5d7efe675"
)
EXPECTED_CM4A_EXACT_REPLAY_DECISION = (
...
def verify_exact_batch_replay_audit():
    if not CM4A_EXACT_REPLAY_AUDIT.is_file():
        raise FileNotFoundError(CM4A_EXACT_REPLAY_AUDIT)

    got = sha256_file(CM4A_EXACT_REPLAY_AUDIT)
    print(
        "CM4A_EXACT_REPLAY_AUDIT",
        got,
        "PASS" if got == EXPECTED_CM4A_EXACT_REPLAY_AUDIT_SHA else "FAIL",
    )
    if got != EXPECTED_CM4A_EXACT_REPLAY_AUDIT_SHA:
        raise RuntimeError("CM4A exact replay audit SHA mismatch.")

    audit = load_json(CM4A_EXACT_REPLAY_AUDIT)

    required = {
        "status": "PASS_DIAGNOSTIC",
        "decision": EXPECTED_CM4A_EXACT_REPLAY_DECISION,
        "replay_batch_size": 8,
        "cudnn_deterministic": True,
        "cudnn_benchmark": False,
        "total_mismatch_slices": 0,
        "total_mismatch_pixels": 0,
        "exact_binary_replay_pass": True,
        "tent": False,
    ...
    for key, expected in required.items():
        ...
        if actual != expected:
            raise RuntimeError(
                f"CM4A exact replay audit field changed: "
                f"{key}={actual!r}, expected {expected!r}"
            )

    if audit.get("cm3_lock_sha256") != EXPECTED_CM3_LOCK_SHA:
        raise RuntimeError("Replay audit CM3 lineage mismatch.")
    if audit.get("cm3_helper_sha256") != EXPECTED_CM3_HELPER_SHA:
        raise RuntimeError("Replay audit CM3 helper lineage mismatch.")

    print("CM4A exact CM3 batch-8 binary replay: PASS")
    print("replay mismatch slices: 0")
    print("replay mismatch pixels: 0")
...
def main():
    ...
    print("===== Q1X CM4A FIX4 SOURCE OOF TENT1 OUTCOME ASSET =====")
    print("STATUS=SOURCE_ONLY_COUNTERFACTUAL_OUTCOME_CONSTRUCTION")
    print("FIX4_CHANGE=USE_FROZEN_CM3_OOF_BASELINE_AFTER_EXACT_BATCH8_REPLAY_PASS")
    print("SOURCE_REPLAY_GATE=batch8_exact_binary_zero_mismatch")
    ...
    exact_replay_audit = verify_exact_batch_replay_audit()
    ...
    for family in FAMILIES:
        ...
        for fold in range(N_FOLDS):
            ...
            # Exact lineage was already re-established prospectively in
            # CM4A Fix3 using the original CM3 validation geometry:
            # batch=8, original fold order, eval mode, fp16 autocast,
            # cudnn deterministic=True, benchmark=False.
            # The audit proved zero binary-mask mismatch over all 15
            # family/fold combinations. The frozen CM3 OOF mask is therefore
            # the authoritative SOURCE baseline here; no batch-1 SOURCE replay
            # is permitted to redefine it.
            print(
                f"{family} fold{fold}: "
                "SOURCE_BASELINE=FROZEN_CM3_OOF_MASK "
                "EXACT_BATCH8_REPLAY_AUDIT=PASS"
            )
    ...
    lock = {
        ...
        "cm4a_exact_batch_replay_audit": {
            "path": str(CM4A_EXACT_REPLAY_AUDIT),
            "sha256": EXPECTED_CM4A_EXACT_REPLAY_AUDIT_SHA,
            "decision": EXPECTED_CM4A_EXACT_REPLAY_DECISION,
            "batch_size": 8,
            "total_mismatch_slices": 0,
            "total_mismatch_pixels": 0,
        ...
        "source_baseline": (
            "frozen CM3 OOF binary mask after exact batch-8 replay audit"
        ),
        "source_patients": 139,
        "source_slices": 3553,
        ...
        "artifacts": {
            ...
            "exact_batch8_replay_audit": str(CM4A_EXACT_REPLAY_AUDIT),
            "exact_batch8_replay_audit_sha256": (
                EXPECTED_CM4A_EXACT_REPLAY_AUDIT_SHA
            ),
        },
        "next_stage": (
    ...
    print("SOURCE baseline: frozen CM3 OOF mask")
    print("Exact CM3 batch-8 replay audit: PASS, mismatch pixels=0")
    print("Tolerance relaxation: NO")
    print("PROMISE12 access: NO")
