...
VERSION = "2026-08-19-Q1-R05C0-v1"
BUILD = "Q1_R05C0_POLYPDB_BACKUP_CONFIRMATORY_TARGET_SELECTION_LOCK"
...
OUTPUT_DIR = (
    ROOT / "outputs"
    / "Q1_R05C0_polypdb_backup_confirmatory_target_selection_lock_v1"
)
...
def evidence_tokens(text: str):
    ...
    # Publication number only counts with explicit publication context.
    for m in re.finditer(re.escape(PUBLICATION_ID), low):
        start = max(0, m.start() - 100)
        end = min(len(low), m.end() + 100)
        context = low[start:end]
        if "arxiv" in context or "polypdb" in context:
...
def scan_disqualifying_assets():
    ...
    for root, path in tqdm(
        ...
        path_hits = evidence_tokens(str(path))
        content_hits = []
...
def run(args):
    ...
    asset_rows, scanned_files = scan_disqualifying_assets()
    ...
    lines = [
        ...
        f"  disqualifying PolypDB assets={len(asset_rows)}",
...
def self_test():
    ...
    assert evidence_tokens("0.2409 0.00045") == []
    assert evidence_tokens("2409.00045") == []
    assert evidence_tokens(
        "arXiv:2409.00045"
    ) == ["2409.00045"]
    assert evidence_tokens(
        "PolypDB paper 2409.00045"
    ) == ["2409.00045", "polypdb"]

    fake_self = Path(
        r"F:\MEDSEG_SAFETTA\docs\Q1_R05C0_polypdb_backup_confirmatory_target_selection_lock_preregistered_protocol_v1.md"
    )
    assert is_self_artifact(fake_self)

    print("DIRECT_POLYPDB_TOKEN_TEST_PASS")
    print("PUBLICATION_CONTEXT_TEST_PASS")
    print("NUMERIC_FALSE_POSITIVE_TEST_PASS")
