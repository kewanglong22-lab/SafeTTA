...
VERSION = "2026-08-19-Q1-R02-v1-fix6"
BUILD = "Q1_R02_SEGFORMER_B0_THREE_SEED_SOURCE_TRAINING_S01_MANIFEST_INTERFACE_FIX6"

ROOT = Path(r"F:\MEDSEG_SAFETTA")
OUTPUT_DIR = ROOT / "outputs" / "Q1_R02_segformer_b0_three_seed_source_training_v1_fix6"

PROTOCOL = (
    ROOT
    / "docs"
    / "Q1_R02_segformer_b0_three_seed_source_training_preregistered_protocol_v1_fix2.md"
)
EXPECTED_PROTOCOL_SHA256 = (
    "6ac20280833567eac54ecaa16b9e6136119ab76086640462828de249f6fd7cb5"
)
...
def load_frozen_s01_source_pairs(root: Path, verify_file_sha: bool = True):
    ...
    for role, row in tqdm(
        all_selected,
        desc="Q1-R02 FIX6 verifying frozen S01 source files",
        unit="pair",
        dynamic_ncols=True,
    ):
        if verify_file_sha:
...
def run_resolve_s01_only(args):
    ...
    print("===== Q1-R02 FIX6 FROZEN S01 MANIFEST RESOLUTION ONLY =====")
    print(f"Script version: {VERSION}")
    print(f"Build: {BUILD}")
    print("Model download=NO")
    print("Training=NO")
...
def run(args):
    ...
    if any(x is not None for x in legacy_overrides):
        raise RuntimeError(
            f"{DECISION_SOURCE}: FIX6 uses the authoritative frozen "
