...
VERSION = "2026-09-04-Q1X-CM2A-v1-fix3"
BUILD = "Q1X_CM2A_PROSTATE158_OFFICIAL_MANIFEST_T2_RESOLVER_AND_SLICE_PREFLIGHT_FIX3"
...
def load_official_source_manifest():
    ...
    rows = []
    # IMPORTANT: do not use DataFrame.itertuples() here. Pandas sanitizes
    # column names beginning with "_" into positional names (e.g. "_1"),
    # which breaks access to our internal metadata columns "_official_split"
    # and "_csv_path". Use record dictionaries so column names remain exact.
...
def main():
    ...
    print("===== Q1X CM2A FIX3 PROSTATE158 OFFICIAL-MANIFEST PREFLIGHT =====")
    print("STATUS=PRETRAINING_SOURCE_SERIES_AND_SAMPLE_UNIT_PREFLIGHT")
    print("FIX3_CHANGE=ROBUST_OFFICIAL_CSV_ROW_ITERATION")
    ...
    lock = {
        ...
        "fix3_reason": (
            "Fix2 correctly used the official train.csv/valid.csv columns, but "
            "pandas.itertuples() sanitized internal column names beginning with "
            "an underscore, causing KeyError('_official_split'). Fix3 uses "
            "to_dict(orient='records') so the authoritative CSV semantics are "
            "unchanged and exact metadata column names are preserved."
        ),
        "source": {
            "dataset": "Prostate158",
