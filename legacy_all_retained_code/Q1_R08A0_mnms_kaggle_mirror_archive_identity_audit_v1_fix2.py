...
r"""
Q1-R08A0 FIX2 — M&Ms Kaggle mirror ZIP identity/completeness audit.

Technical fix:
- persists a successful live Kaggle listing to a stable local cache;
- formal execution reuses that cache without re-authenticating.
...
VERSION = "2026-08-19-Q1-R08A0-v1-fix2"
BUILD = "Q1_R08A0_MNMS_KAGGLE_MIRROR_ARCHIVE_IDENTITY_AUDIT_FIX2"

ROOT = Path(r"F:\MEDSEG_SAFETTA")

PROTOCOL = (
    ROOT / "docs"
    / "Q1_R08A0_mnms_kaggle_mirror_archive_identity_audit_preregistered_protocol_v1_fix2.md"
)
EXPECTED_PROTOCOL_SHA256 = "cedfcba4d8b654436fdb4b79fd980802afb612206404829194d92d65562d878d"
...
OUTPUT_DIR = (
    ROOT / "outputs"
    / "Q1_R08A0_mnms_kaggle_mirror_archive_identity_audit_v1_fix2"
)
...
def persist_manifest_rows(path: Path, rows: list[dict]):
    validate_manifest_rows(rows)
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(f, fieldnames=["remote_path", "bytes"])
        writer.writeheader()
        writer.writerows(rows)

    reread = read_manifest_file(path)
    if reread != rows:
        raise RuntimeError("Persisted Kaggle manifest round-trip mismatch.")
...
def resolve_manifest(explicit: Path | None):
    ...
    if PERSISTENT_REMOTE_MANIFEST.is_file():
        ...
        return rows, "persistent_local_manifest", str(PERSISTENT_REMOTE_MANIFEST)

    if HISTORICAL_REMOTE_MANIFEST.is_file():
        rows = read_manifest_file(HISTORICAL_REMOTE_MANIFEST)
        persist_manifest_rows(PERSISTENT_REMOTE_MANIFEST, rows)
        return rows, "historical_manifest_persisted", str(PERSISTENT_REMOTE_MANIFEST)

    rows = fetch_kaggle_manifest()
    validate_manifest_rows(rows)
    persist_manifest_rows(PERSISTENT_REMOTE_MANIFEST, rows)
    return rows, "live_kaggle_api_snapshot_persisted", str(PERSISTENT_REMOTE_MANIFEST)
...
def self_test():
    ...
    assert str(PERSISTENT_REMOTE_MANIFEST).lower().endswith("_kaggle_remote_manifest_snapshot.csv")
    print("PERSISTENT_MANIFEST_CACHE_PATH_TEST_PASS")
    print("LOCAL_OR_LIVE_MANIFEST_FALLBACK_TEST_PASS")
    print("SELF_TEST_PASS")


def preflight(args):
    validate_sha(PROTOCOL, EXPECTED_PROTOCOL_SHA256, "R08A0 FIX2 protocol")
    ...
    print(f"persistent_manifest_cache={PERSISTENT_REMOTE_MANIFEST}")
    print(f"persistent_manifest_cache_exists={'YES' if PERSISTENT_REMOTE_MANIFEST.is_file() else 'NO'}")
...
def parse_args():
    p = argparse.ArgumentParser(
        description=(
            "Q1-R08A0 FIX2: archive-only M&Ms Kaggle mirror audit with "
            "persistent Kaggle manifest caching."
        )
    )
    p.add_argument(
        "--zip",
