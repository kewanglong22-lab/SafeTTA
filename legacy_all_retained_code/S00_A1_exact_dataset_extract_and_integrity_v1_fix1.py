...
VERSION = "2026-08-17-S00-A1-v1-fix1"
BUILD = "S00_A1_FIX1_ACCEPT_01_OR_0255_BINARY_MASKS"
...
def inspect_mask(path: Path):
    """
    Validate a raw segmentation mask without modifying it.

    Accepted binary encodings:
      - {0, 1}
      - {0, 255}

    Single-valued {0}, {1}, or {255} is also recognized as a technically
    binary encoding, but foreground-fraction diagnostics remain available.
    Any other multi-valued encoding (e.g. {0,128,255}) is rejected.
    """
    ...
    binary = unique_set in allowed_encodings
    ...
    return size, mode, unique, binary, encoding, fg_hash, fg_fraction
    ...
    try:
        with zipfile.ZipFile(archive, "r") as zf:
            ...
            for split, dataset, pair_key, image_member, mask_member in tqdm(
                ...
                mask_size, mask_mode, mask_unique, mask_binary, mask_encoding, mask_fg_sha, fg_fraction = inspect_mask(mask_dest)
                ...
                if not mask_binary:
                    raise RuntimeError(
                        ...
                        f"values={mask_unique[:50]}; accepted encodings are {{0,1}} or {{0,255}}"
                    )
                ...
                rows.append({
                    ...
                    "mask_binary_encoding_valid": True,
        ...
        for (split, dataset), expected in EXPECTED_COUNTS.items():
            ...
            summary_rows.append({
                ...
                "binary_masks": sum(bool(r["mask_binary_encoding_valid"]) for r in group),
                "mask_encoding_0_1": sum(r["mask_encoding"] == "0_1" for r in group),
                "mask_encoding_0_255": sum(r["mask_encoding"] == "0_255" for r in group),
        ...
        manifest_fields = [
            ...
            "mask_unique_values", "mask_binary_encoding_valid", "mask_encoding",
        ...
        write_csv(
            ...
            [
                ...
                "binary_masks", "mask_encoding_0_1", "mask_encoding_0_255", "mean_mask_foreground_fraction"
            ],
        )
        write_csv(
        ...
        audit = {
            ...
            "all_masks_binary_encoding_valid": all(bool(r["mask_binary_encoding_valid"]) for r in rows),
        ...
        for row in summary_rows:
            summary_lines.append(
                ...
                f"enc01={row['mask_encoding_0_1']}, enc0255={row['mask_encoding_0_255']}"
            )
        ...
        summary_lines += [
            ...
            "All masks binary (accepted raw encodings {0,1} or {0,255}): YES",
...
def self_test():
    ...
    # Binary mask encoding tests: both conventional encodings must pass,
    # while a genuinely multi-valued mask must fail.
    import tempfile
    with tempfile.TemporaryDirectory() as td:
        ...
        _, _, u01, b01, e01, _, _ = inspect_mask(p01)
        assert u01 == [0, 1] and b01 and e01 == "0_1"

        p255 = td / "mask255.png"
        Image.fromarray(np.array([[0, 255], [255, 0]], dtype=np.uint8)).save(p255)
        _, _, u255, b255, e255, _, _ = inspect_mask(p255)
        assert u255 == [0, 255] and b255 and e255 == "0_255"

        pbad = td / "mask_bad.png"
        Image.fromarray(np.array([[0, 128], [255, 0]], dtype=np.uint8)).save(pbad)
        _, _, ubad, bbad, ebad, _, _ = inspect_mask(pbad)
        assert ubad == [0, 128, 255] and (not bbad) and ebad == "INVALID"

    print("MASK_01_ENCODING_TEST_PASS")
    print("MASK_0255_ENCODING_TEST_PASS")
    print("MASK_MULTIVALUE_REJECTION_TEST_PASS")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "S00-A1 fix1 exact parser + selective extraction + integrity audit "
