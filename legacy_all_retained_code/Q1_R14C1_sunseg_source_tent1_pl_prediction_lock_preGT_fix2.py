...
"""
Q1_R14C1_sunseg_source_tent1_pl_prediction_lock_preGT_fix2.py
...
def migrate_verified_fix1_state_if_available(new_output_dir, state):
    """
    Reuse only atomically locked Fix1 states that pass strict SHA/cardinality
    validation, then revalidate the copied artifacts in the Fix2 directory.
    """
    if not LEGACY_FIX1_OUTPUT_DIR.exists():
        return None

    new_pred, new_idx, new_lock = state_paths(new_output_dir, state)
    if any(p.exists() for p in (new_pred, new_idx, new_lock)):
        return None

    old_rows = load_completed_state(
        LEGACY_FIX1_OUTPUT_DIR,
        state,
    )
    if old_rows is None:
    ...
    old_pred, old_idx, old_lock = state_paths(
        LEGACY_FIX1_OUTPUT_DIR,
        state,
    )

    new_pred.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(old_pred, new_pred)
    shutil.copy2(old_idx, new_idx)
    shutil.copy2(old_lock, new_lock)

    copied_rows = load_completed_state(
        new_output_dir,
        state,
    )
    if copied_rows is None:
        raise RuntimeError(
            f"Fix1 migration verification returned None: "
            f"{state['model_state_id']}"
        )

    print(
        f"MIGRATED SHA-VERIFIED Fix1 state -> Fix2: "
        f"{state['model_state_id']}"
    )
    return copied_rows
...
def run(args):
    ...
    if args.output_dir.exists() and not args.resume:
        raise FileExistsError(
            ...
            f"technical interruption or SHA-verified migration from R14C1 Fix1."
        )
    ...
    print("\n===== R14C1 FIX2 EXECUTION =====")
    print("FIX2_CHANGE=PASS_R03_EXPLICITLY_TO_R05D3_SEGFORMER")
    print("SCIENTIFIC_PROTOCOL_CHANGE_FROM_FIX1=NO")
    print("FIX1_COMPLETED_STATE_REUSE=SHA_VERIFIED_COPY_ONLY")
    ...
    for state_no, state in enumerate(panel, start=1):
        ...
        if completed is None:
            completed = migrate_verified_fix1_state_if_available(
                args.output_dir,
                state,
            )
        ...
        if completed is not None:
            if not args.resume:
                raise RuntimeError(
                    "Completed/migrated state exists but --resume was not "
                    "requested. Rerun Fix2 with --resume."
                )
    ...
    lock = {
        ...
        "runner_fix": {
            "version": "R14C1_FIX2",
            "change": (
                "Pass r03 explicitly as the fourth dependency to R05D3 "
                "run_segformer_state; no action semantic change."
            ),
            "fix1_completed_state_reuse": (
                "only through SHA/cardinality-validated artifact migration"
            ),
        },
...
def parse_args():
    ...
    p.add_argument(
        ...
        help=(
            ...
            "technical interruption or SHA-verified migration from the interrupted Fix1 run."
        ),
    )
