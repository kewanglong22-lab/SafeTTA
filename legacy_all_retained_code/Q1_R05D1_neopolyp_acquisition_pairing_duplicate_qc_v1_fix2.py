...
VERSION = "2026-08-19-Q1-R05D1-v1-fix2"
BUILD = "Q1_R05D1_NEOPOLYP_ACQUISITION_PAIRING_DUPLICATE_QC_KAGGLE_LAUNCHER_FIX2"
...
def resolve_kaggle_launcher() -> Path:
    """
    Resolve the Windows Kaggle console-script launcher.

    The official kaggle package exposes the console entry point
    `kaggle = kaggle.cli:main`. On Windows/Conda, pip normally installs
    kaggle.exe under <environment>\\Scripts.
    """
    candidates = []

    which_kaggle = shutil.which("kaggle")
    if which_kaggle:
        candidates.append(Path(which_kaggle))

    env_scripts = Path(sys.prefix) / "Scripts"
    candidates.extend([
        env_scripts / "kaggle.exe",
        env_scripts / "kaggle",
    ])
    ...
    raise RuntimeError(
        "Kaggle console-script launcher was not found.\n"
        f"Expected for this Python environment: {env_scripts / 'kaggle.exe'}\n"
        "Searched:\n"
        f"{searched}\n"
        "Install/reinstall Kaggle into this exact environment with:\n"
...
def run_kaggle_download(raw_root: Path) -> Path:
    ...
    kaggle_exe = resolve_kaggle_launcher()
    ...
    print("Kaggle launcher:", kaggle_exe)
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            "Kaggle download failed. Confirm authentication and competition "
            "access/rules. This script does not use `python -m kaggle` and "
            "does not use unofficial mirrors."
        )
...
def preflight():
    ...
    kaggle_launcher = resolve_kaggle_launcher()
    print(f"kaggle_launcher={kaggle_launcher}")
    print("kaggle_launcher_resolved=YES")
...
def self_test():
    ...
    import inspect
    launcher_source = inspect.getsource(run_kaggle_download)
    resolver_source = inspect.getsource(resolve_kaggle_launcher)

    assert 'Path(sys.prefix) / "Scripts"' in resolver_source
    assert '"kaggle.exe"' in resolver_source
    assert 'sys.executable, "-m", "kaggle"' not in launcher_source
    ...
    print("WINDOWS_KAGGLE_LAUNCHER_RESOLUTION_TEST_PASS")
    print("NO_PYTHON_MINUS_M_KAGGLE_FALLBACK_TEST_PASS")
