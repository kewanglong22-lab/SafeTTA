...
OUTPUT_DIR = OUT / "Q1_R13D_pl_conf90_frozen_safety_transfer_evaluation_fix1_v1"
...
def load_locked_features(
    ...
    if "training_seed_morph" in joined.columns:
        seed_ok = joined["training_seed_morph"].astype(str) == joined["training_seed_repr"].astype(str)
        if not seed_ok.all():
...
def build_score_panel(r13c_panel: pd.DataFrame, frozen_threshold: float):
    ...
    for c in ("training_seed", "checkpoint_sha256"):
        a = merged[f"{c}_score"].astype(str)
        b = merged[f"{c}_r13c"].astype(str)
        if not a.equals(b):
            raise RuntimeError(f"Metadata mismatch after merge: {c}")
        merged[c] = merged[f"{c}_score"]
...
def main():
    ...
    print("===== R13D PL-CONF90 FROZEN SAFETY TRANSFER EVALUATION FIX1 =====")
