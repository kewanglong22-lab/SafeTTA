...
OUTPUT_DIR = ROOT / "outputs" / "Q1_R11D_tent2_frozen_safety_transfer_evaluation_fix1_v1"
...
def build_a2_panel(r05d4, d4, gt_masks):
    ...
    req = [
        "sample_id","model_family","model_state_id","training_seed",
        "checkpoint_sha256","state_prediction_npz","state_lock_json","state_row_index",
    ]
    ...
    for state_id, g in man.groupby("model_state_id", sort=True):
        ...
        if len(npzs) != 1 or len(locks) != 1:
            raise RuntimeError(f"{state_id}: ambiguous assets")
        npz_path, lock_path = Path(npzs[0]), Path(locks[0])
        ...
        for r in tqdm(
            g.sort_values("state_row_index").itertuples(index=False),
...
def run(args):
    print("===== R11D TENT2 FROZEN SAFETY TRANSFER EVALUATION FIX1 =====")
