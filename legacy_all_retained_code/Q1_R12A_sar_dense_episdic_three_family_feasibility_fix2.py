...
R12A_FIX1_LOCK = (
    OUT/"Q1_R12A_sar_dense_episdic_three_family_feasibility_fix1_v1"
    /"R12A_SAR_DENSE_FEASIBILITY_LOCK.json"
)
R12A_FIX1_SHA = "8de2960cf37e93bd5e090d00a080d5d8eb776153be34b48f4cc102c6474e8c27"

OUTDIR = OUT/"Q1_R12A_sar_dense_episdic_three_family_feasibility_fix2_v1"
...
def reliable_first(e):
    m=e<MARGIN
    n=int(m.sum().detach().cpu())
    total=int(m.numel())
    return (None,n,total,m) if n==0 else (e[m].mean(),n,total,m)

def reliable_second(e_second, first_mask):
    # Official SAR ordering: second-pass entropy is first restricted to the
    # units selected by the first-pass reliable filter, then thresholded again.
    restricted=e_second[first_mask]
    if int(restricted.numel())==0:
        return None,0
    m2=restricted<MARGIN
    n2=int(m2.sum().detach().cpu())
    return (None,n2) if n2==0 else (restricted[m2].mean(),n2)
...
def lineage():
    for p in (R05D3,R11D,POLYPGEN,R12A_FIX1_LOCK):
    ...
    if sha(R12A_FIX1_LOCK)!=R12A_FIX1_SHA:
        raise RuntimeError("R12A Fix1 lock SHA mismatch")
    fix1=json.loads(R12A_FIX1_LOCK.read_text(encoding="utf-8"))
    if fix1.get("decision")!="SAR_DENSE_EPISODIC_THREE_FAMILY_TECHNICAL_FEASIBILITY_PASS":
        raise RuntimeError("R12A Fix1 did not PASS")
    return {
        "r05d3":sha(R05D3),
        "r11d":sha(R11D),
        "polypgen_lock":sha(POLYPGEN),
        "r12a_fix1_lock":sha(R12A_FIX1_LOCK),
    }
...
def binary_family(r05,state,targets,dev,spack):
    ...
    for i in tqdm(PROBES,desc=f"R12A {fam} {sd}",unit="case",dynamic_ncols=True):
        ...
        z1=logits(x); l1,n1,tot,first_mask=reliable_first(bent(z1))
        ...
        if l1 is not None:
            ...
            z2=logits(x); l2,n2=reliable_second(bent(z2),first_mask)
...
def segformer(r05,r03,state,targets,dev,spack,ctx=None):
    ...
    for i in tqdm(PROBES,desc=f"R12A SegFormer {sd}",unit="case",dynamic_ncols=True):
        ...
        z1,_=r03.segformer_logits_and_z(model,x,F); l1,n1,tot,first_mask=reliable_first(cent(z1))
        upd=False; l1v=l2v=math.nan; gn=pn=0.; n2=0
        if l1 is not None:
            if not torch.isfinite(l1): raise RuntimeError("loss1")
            l1v=float(l1.detach().cpu()); l1.backward(); gn,pn=sam.first()
            z2,_=r03.segformer_logits_and_z(model,x,F); l2,n2=reliable_second(cent(z2),first_mask)
            if l2 is not None and torch.isfinite(l2):
                l2v=float(l2.detach().cpu()); l2.backward(); sam.second(); upd=True
            else: sam.cancel()
...
def main():
    ...
    if a.self_test:
        ...
        import torch
        e1=torch.tensor([0.10,0.20,0.40,0.05])
        l1,n1,total,m1=reliable_first(e1)
        e2=torch.tensor([0.30,0.10,0.01,0.25])
        l2,n2=reliable_second(e2,m1)
        assert n1==3 and n2==2 and n2<=n1
        print("SECOND_PASS_NESTED_FILTER_TEST_PASS")
        print("SELF_TEST_PASS"); return
    print("===== R12A SAR-DENSE EPISODIC THREE-FAMILY FEASIBILITY FIX2 =====")
    ...
    fs=(df.groupby("model_family",as_index=False)
        .agg(probe_rows=("updated","size"),updated_rows=("updated","sum"),
             ...
             parameter_delta_max=("parameter_max_abs_delta","max")))
    nested_second_pass=bool(
        (df["reliable_pixels_second"]<=df["reliable_pixels_first"]).all()
    )
    ok=bool(
        (df.source_parity==1).all()
        and nested_second_pass
        and (ss.updated_cases>=1).all()
        and (ss.parameter_delta_max>0).all()
    )
    ...
    print("\nsecond-pass subset of first-pass reliable units:",nested_second_pass)
    print("model-recovery EMA/reset: DISABLED (episodic source reset per case)")
    print("action prediction: SAME-CASE POST-SAM-UPDATE")
    print("\nDecision:",dec)
    ...
    lock={
          ...
          "margin":MARGIN,
          "dense_sar_semantics":{
              "second_pass_restricted_to_first_pass_reliable_units":True,
              "second_pass_subset_check_passed":nested_second_pass,
              "model_recovery_ema_reset":False,
              "model_recovery_disable_reason":"episodic source reset before every physical case",
              "action_prediction":"same-case post-SAM-update segmentation",
              "official_online_first_forward_output_as_action_prediction":False
          },
