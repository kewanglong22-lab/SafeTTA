...
VERSION = "2026-09-05-Q1X-CM4D-v1-fix2"
BUILD = "Q1X_CM4D_FINAL_ALL_SOURCE_SEGMENTATION_MODELS_LOCK_FIX2"
...
FIX1_UNET_FINAL = CM4D_FIX1_DIR / "models" / "UNet" / "final.pt"
FIX1_UNET_COMPLETE = CM4D_FIX1_DIR / "models" / "UNet" / "COMPLETE.json"
EXPECTED_FIX1_UNET_FINAL_SHA = (
    "006932908c110b0e16efe58d5d218266d395410263024dcfd612910e325642bb"
)
...
def set_deeplab_singleton_safe_aspp_bn(model, nn):
    """
    For an actual training batch of size 1, the ASPP global-pooling branch has
    shape [1,C,1,1], for which training-mode BatchNorm2d is undefined.

    Freeze only that singleton-unsafe BN's running-stat update for this one
    forward pass by switching it to eval mode. Affine parameters remain
    trainable and gradients still flow through them.

    This condition never occurred in CM3 SOURCE OOF training because every
    training fold remainder was >1. It occurs only for the all-SOURCE cardinality
    3553 with batch_size=8, whose tail batch has size 1.
    """
    found = []
    ...
    if not found:
        raise RuntimeError(
            "DeepLab singleton batch encountered but ASPP pooled BN "
            "was not found."
        )
...
def restore_deeplab_aspp_bn_train(model, nn):
    found = []
    ...
    if not found:
        raise RuntimeError("Unable to restore DeepLab ASPP pooled BN.")

    return found


def import_valid_fix1_unet():
    """
    Reuse the already completed Fix1 U-Net final model rather than retraining it.
    The DeepLab singleton issue is architecture-specific and occurred only after
    U-Net had been fully trained, saved, hashed, and sanity-evaluated.
    """
    if not FIX1_UNET_FINAL.is_file():
        raise FileNotFoundError(FIX1_UNET_FINAL)
    if not FIX1_UNET_COMPLETE.is_file():
        raise FileNotFoundError(FIX1_UNET_COMPLETE)

    got = sha256_file(FIX1_UNET_FINAL)
    print(
        "FIX1_UNET_FINAL",
        got,
        "PASS" if got == EXPECTED_FIX1_UNET_FINAL_SHA else "FAIL",
    )
    if got != EXPECTED_FIX1_UNET_FINAL_SHA:
        raise RuntimeError("Fix1 U-Net final checkpoint SHA mismatch.")

    done = load_json(FIX1_UNET_COMPLETE)
    ...
    for key, expected in required.items():
        if done.get(key) != expected:
            raise RuntimeError(
                f"Fix1 U-Net COMPLETE changed: "
                f"{key}={done.get(key)!r}, expected={expected!r}"
            )

    if done.get("final_checkpoint_sha256") != EXPECTED_FIX1_UNET_FINAL_SHA:
        raise RuntimeError("Fix1 U-Net COMPLETE checkpoint SHA mismatch.")

    # Preserve the original paths and hashes explicitly in the Fix2 lineage.
    imported = dict(done)
    imported["imported_from_cm4d_fix1"] = True
    imported["imported_complete_path"] = str(FIX1_UNET_COMPLETE)
    imported["imported_complete_sha256"] = sha256_file(FIX1_UNET_COMPLETE)

    print(
        "UNet: IMPORT VALID FIX1 COMPLETE "
        f"epochs={done['fixed_epochs']} "
        f"sanity_dice="
        f"{done['all_source_training_sanity_mean_patient_dice']:.6f}"
    )
...
def train_final_family(
    ...
    singleton_safe_fallback_total = 0
    ...
    for epoch in range(start_epoch, final_epochs + 1):
        ...
        running = 0.0
        n_seen = 0
        singleton_safe_fallback_epoch = 0
        ...
        for batch in pbar:
            ...
            singleton_safe_bn_names = []

            if family == "DeepLabV3_R50" and int(x.shape[0]) == 1:
                singleton_safe_bn_names = (
                    set_deeplab_singleton_safe_aspp_bn(model, nn)
                )
                singleton_safe_fallback_epoch += 1
                singleton_safe_fallback_total += 1
            ...
            try:
                ...
                if singleton_safe_bn_names:
                    restore_deeplab_aspp_bn_train(model, nn)
        ...
        history.append(
            {
                ...
                "singleton_safe_aspp_bn_fallback_batches": (
                    singleton_safe_fallback_epoch
                ),
            }
        )
        ...
        print(
            ...
            f"singleton_safe_bn_batches={singleton_safe_fallback_epoch}"
        )
    ...
    torch.save(
        {
            ...
            "source_only": True,
            "promises12_access": False,
            "deeplab_singleton_safe_aspp_bn": (
                family == "DeepLabV3_R50"
            ),
            "deeplab_singleton_safe_fallback_total": (
                singleton_safe_fallback_total
            ),
        },
        final_path,
    )
    ...
    patient_df.to_csv(patient_path, index=False)
        ...
        "deeplab_singleton_rule": (
            "Only when actual batch size == 1: ASPP pooled BatchNorm2d "
            "uses source running statistics (eval mode) for that forward; "
            "affine parameters remain trainable; all samples retained."
...
def main():
    ...
    print(
        "===== Q1X CM4D FIX2 FINAL ALL-SOURCE SEGMENTATION MODEL TRAINING ====="
    )
    ...
    print("FIX2_CHANGE=DEEPLAB_SINGLETON_SAFE_ASPP_BN")
    print("DROP_LAST=NO")
    print("DUPLICATE_SAMPLES=NO")
    print("BATCH_SIZE_CHANGE=NO")
    ...
    # Fix1 U-Net was fully completed before the architecture-specific DeepLab
    # singleton failure. Reuse it under an exact checkpoint SHA gate.
    results["UNet"] = import_valid_fix1_unet()

    for family in ["DeepLabV3_R50", "SegFormer_B0"]:
        print(
    ...
    lock = {
        ...
        "fix2_engineering_correction": {
            "issue": (
                "DeepLabV3-R50 all-SOURCE tail batch is size 1 because "
                "3553 mod 8 = 1; ASPP pooled training-mode BatchNorm2d "
                "receives [1,C,1,1] and is undefined."
            ),
            "correction": (
                "For DeepLab only and only when actual batch size == 1, "
                "switch the ASPP pooled BatchNorm2d to eval mode for that "
                "forward, retaining source running statistics while affine "
            ...
            "valid_fix1_unet_reused": True,
            "fix1_unet_checkpoint_sha256": EXPECTED_FIX1_UNET_FINAL_SHA,
        },
        "duration_selection": {
    ...
    print("Fix2 DeepLab singleton-safe ASPP BN: ENABLED only for batch size 1")
    print("Drop last: NO")
    print("Duplicate samples: NO")
    print("Batch-size change: NO")
