...
VERSION = "2026-08-18-S05-D-v1-fix2"
BUILD = "S05_D_FIX2_GLOBAL_LOCK_PROVENANCE_SCOPE_AND_RESUME"
...
PROTOCOL = ROOT / "docs" / "S05_D_deeplab_locked_target_replication_protocol_v1_fix2.md"
EXPECTED_PROTOCOL_SHA256 = "e5b6f7b25be18c7924fb305cd0544723406e1c8ccf3b7346feda96d299f705e2"
...
EXPECTED_S05B_TRAINING_HELPER_SHA256 = "7d0fd5192a9af40dbae0aeac692a65641745fbef2e986f1cd4e163a59ad93a31"
...
OUTPUT_DIR = (
    ROOT / "outputs"
    / "S05_D_deeplab_locked_safettta_target_replication_v1_fix2"
)
LEGACY_FIX1_BUILD_DIR = (
    ROOT / "outputs"
    / "S05_D_deeplab_locked_safettta_target_replication_v1_fix1__building"
)
...
def produce_seed_no_label(
    ...
    for i, row in pbar:
        ...
        x = helper.image_to_model_tensor(training_helper, image).to(
            device,
            non_blocking=True,
        )
...
def build_global_no_label_lock(
    ...
    global_lock = {
        ...
        "gate_artifact_sha256": EXPECTED_GATE_SHA256,
        "s05c_helper_sha256": EXPECTED_S05C_HELPER_SHA256,
        "s05b_training_helper_sha256": EXPECTED_S05B_TRAINING_HELPER_SHA256,
        "helper_composition": "S05-D -> S05-C SafeTTA -> S05-B FIX2 DeepLab",
        "reused_verified_fix1_no_label_seed_locks": bool(migrated_from_fix1),
        "checkpoint_sha256": {
...
def migrate_legacy_fix1_build_if_requested(build_dir: Path, resume: bool):
    if not resume:
        return False
    if build_dir.exists():
        return False
    if not LEGACY_FIX1_BUILD_DIR.exists():
        return False
    LEGACY_FIX1_BUILD_DIR.rename(build_dir)
    return True
...
def run(args):
    ...
    # FIX1: S05-C owns SafeTTA utilities; architecture/model forwarding
    # comes from the separately frozen S05-B FIX2 helper.
    if not hasattr(helper, "import_training_helper"):
        raise RuntimeError(
            "Frozen S05-C helper does not expose import_training_helper()."
        )
    ...
    for required_name in (
        ...
        if not hasattr(training_helper, required_name):
            raise RuntimeError(
                f"Frozen S05-B FIX2 helper missing {required_name}"
            )
    ...
    migrated_from_fix1 = migrate_legacy_fix1_build_if_requested(
        build_dir,
        args.resume,
    )
    if migrated_from_fix1:
        print(f"[MIGRATE] FIX1 locked no-label build -> {build_dir}")
    ...
    print("Helper composition: S05-D -> S05-C SafeTTA -> S05-B FIX2 DeepLab")
    print(f"S05-B FIX2 helper SHA256: {EXPECTED_S05B_TRAINING_HELPER_SHA256}")
    print("Phase A target images opened before model construction=NO")
    ...
    for seed in SEEDS:
        ...
        produce_seed_no_label(
            helper=helper,
            training_helper=training_helper,
            gate=gate,
            eval_rows=eval_rows,
            seed=seed,
            device=device,
    ...
    global_lock_path, global_lock_sha = build_global_no_label_lock(
        ...
        eval_rows,
        checkpoint_hashes,
        migrated_from_fix1=migrated_from_fix1,
    )
...
def self_test():
    # FIX1 helper composition contract.
    class FakeTrainingHelper:
        def build_deeplab(self, pretrained_backbone=False):
            return "MODEL", "NONE"

        def image_to_tensor(self, image):
            return image

        def deeplab_logits(self, model, x):
            return x

    class FakeS05CHelper:
        def import_training_helper(self):
            return FakeTrainingHelper()

    fake_s05c = FakeS05CHelper()
    fake_training = fake_s05c.import_training_helper()
    assert hasattr(fake_training, "build_deeplab")
    assert hasattr(fake_training, "image_to_tensor")
    assert hasattr(fake_training, "deeplab_logits")
    assert not hasattr(fake_s05c, "build_deeplab")
    ...
    # FIX2 exact global-lock construction path.
    import tempfile
    with tempfile.TemporaryDirectory() as td:
        td = Path(td)
        eval_rows_toy = [{
            "sample_id": "toy_target",
            "s01_role": "unseen_locked",
            "dataset": "CVC-ColonDB",
        }]

        for seed in SEEDS:
            paths = seed_artifact_paths(td, seed)
            ...
            np.savez_compressed(
                paths["npz"],
                source_logits=np.zeros((1, IMAGE_SIZE, IMAGE_SIZE), dtype=np.float16),
                tent_logits=np.zeros((1, IMAGE_SIZE, IMAGE_SIZE), dtype=np.float16),
                sample_ids=np.asarray(["toy_target"], dtype=str),
                roles=np.asarray(["unseen_locked"], dtype=str),
                datasets=np.asarray(["CVC-ColonDB"], dtype=str),
                seed=np.asarray([seed], dtype=np.int64),
            )
            ...
            paths["lock"].write_text(
                json.dumps(seed_lock, indent=2),
                encoding="utf-8",
            )
            assert validate_seed_no_label_lock(paths, seed, eval_rows_toy) is not None

        global_path, global_sha = build_global_no_label_lock(
            td,
            eval_rows_toy,
            {seed: CHECKPOINTS[seed]["sha256"] for seed in SEEDS},
            migrated_from_fix1=True,
        )
        assert global_path.exists()
        assert file_sha256(global_path) == global_sha
        obj = json.loads(global_path.read_text(encoding="utf-8"))
        assert obj["s05b_training_helper_sha256"] == EXPECTED_S05B_TRAINING_HELPER_SHA256
        assert obj["reused_verified_fix1_no_label_seed_locks"] is True
    ...
    print("HELPER_COMPOSITION_TEST_PASS")
    print("GLOBAL_LOCK_CONSTRUCTION_TEST_PASS")
    print("FIX1_LOCK_REUSE_PROVENANCE_TEST_PASS")
