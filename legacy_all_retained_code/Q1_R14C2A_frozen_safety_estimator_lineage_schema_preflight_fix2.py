...
def artifact_reference_audit(lock_obj, artifact_path: Path, digest: str):
    """
    Tri-state artifact-lineage audit.

    Historical R10L0 may reference the frozen artifact by path/name but not
    contain a SHA256 field. Absence of a historical SHA field is different
    from a SHA mismatch.

    Returns:
      name_hit:
          artifact filename is referenced somewhere in the lock.
      exact_sha_hit:
          the current digest occurs exactly somewhere in the lock.
      sha_like_values:
          all 64-hex strings present in the lock, for audit transparency.
      status:
          VERIFIED_BY_HISTORICAL_SHA
          or REFERENCED_BUT_HISTORICAL_SHA_ABSENT
          or NOT_REFERENCED
    """
    strings = flatten_strings(lock_obj)
    name_hit = any(artifact_path.name in s for s in strings)
    exact_sha_hit = any(
        digest.lower() == s.strip().lower()
        for s in strings
    )

    sha_like_values = sorted({
        s.strip().lower()
        for s in strings
        if re.fullmatch(r"[0-9a-fA-F]{64}", s.strip())
    })

    if exact_sha_hit:
        status = "VERIFIED_BY_HISTORICAL_SHA"
    elif name_hit:
        status = "REFERENCED_BUT_HISTORICAL_SHA_ABSENT"
    else:
        status = "NOT_REFERENCED"

    return {
        "name_hit": name_hit,
        "exact_sha_hit": exact_sha_hit,
        "sha_like_values": sha_like_values,
        "status": status,
    }
...
def main():
    ...
    print(
        "===== Q1 R14C2A FIX2 FROZEN SAFETY ESTIMATOR "
        "LINEAGE/SCHEMA PREFLIGHT ====="
    )
    print("FIX2_CHANGE=ABSENT_HISTORICAL_ARTIFACT_SHA_IS_NOT_A_MISMATCH")
    print("SCIENTIFIC_PROTOCOL_CHANGE_FROM_FIX1=NO")
    print("DOWNSTREAM_ARTIFACT_SHA_FREEZE=YES")
    ...
    for name, path, digest in (
        ...
        aa = artifact_reference_audit(
            r10, path, digest
        )
        artifact_audits[name] = aa

        print(
            f"{name} R10L0 lineage: "
            f"name={aa['name_hit']} "
            f"exact_sha={aa['exact_sha_hit']} "
            f"status={aa['status']}"
        )

        if aa["sha_like_values"]:
            print(
                f"{name} lock-level SHA-like values "
                f"(audit only): {aa['sha_like_values']}"
            )
        else:
            print(
                f"{name} lock-level SHA-like values: NONE"
            )
        ...
        # Important Fix2 distinction:
        # historical SHA absence does not imply artifact mismatch.
        # The current exact SHA is frozen by this R14C2A audit for all
        # downstream scoring stages.
        if aa["exact_sha_hit"]:
            print(
                f"{name}: historical SHA verification PASS"
            )
        else:
            print(
                f"{name}: historical SHA field absent/not matching any "
                f"stored 64-hex value; NOT treated as corruption because "
                f"the frozen lock references the exact artifact name. "
                f"Current SHA is now frozen for downstream R14C2."
            )
    ...
    audit = {
        ...
        "r10l0": {
            ...
            "artifact_lineage_audit": artifact_audits,
            "downstream_sha_freeze": {
                "PCA": pca_sha,
                "HEAD": head_sha,
                "THRESH": thresh_sha,
                "rule": (
                    "These exact bytes are frozen for all subsequent R14C2 "
                    "SUN scoring. Historical R10L0 SHA absence is recorded "
                    "as absence, not interpreted as mismatch."
                ),
            },
            "pca_schema": pca_info,
    ...
    print("\n===== R14C2A FINAL =====")
    print(
        "R10L0 historical SHA policy="
        "ABSENT_IS_NOT_MISMATCH_CURRENT_BYTES_FROZEN_DOWNSTREAM"
    )
    print("PCA downstream SHA freeze=", pca_sha)
    print("HEAD downstream SHA freeze=", head_sha)
    print("THRESH downstream SHA freeze=", thresh_sha)
    print("status=", status)
