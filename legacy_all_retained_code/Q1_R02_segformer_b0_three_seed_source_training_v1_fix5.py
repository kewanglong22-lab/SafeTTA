...
VERSION = "2026-08-19-Q1-R02-v1-fix5"
BUILD = "Q1_R02_SEGFORMER_B0_THREE_SEED_SOURCE_TRAINING_S01_MANIFEST_INTERFACE_FIX5"

ROOT = Path(r"F:\MEDSEG_SAFETTA")
OUTPUT_DIR = ROOT / "outputs" / "Q1_R02_segformer_b0_three_seed_source_training_v1_fix5"

PROTOCOL = (
    ROOT
    / "docs"
    / "Q1_R02_segformer_b0_three_seed_source_training_preregistered_protocol_v1_fix1.md"
)
EXPECTED_PROTOCOL_SHA256 = (
    "ec588aad578ea8c54f749ede8ff7b9a53bb37863de4c98ebdbc16ea953ce283d"
)
...
def load_frozen_s01_source_pairs(root: Path, verify_file_sha: bool = True):
    ...
    for role, row in tqdm(
        all_selected,
        desc="Q1-R02 FIX5 verifying frozen S01 source files",
        unit="pair",
        dynamic_ncols=True,
    ):
        if verify_file_sha:
...
def run_resolve_s01_only(args):
    ...
    print("===== Q1-R02 FIX5 FROZEN S01 MANIFEST RESOLUTION ONLY =====")
    print(f"Script version: {VERSION}")
    print(f"Build: {BUILD}")
    print("Model download=NO")
    print("Training=NO")
...
def run(args):
    ...
    if any(x is not None for x in legacy_overrides):
        raise RuntimeError(
            f"{DECISION_SOURCE}: FIX5 uses the authoritative frozen "
