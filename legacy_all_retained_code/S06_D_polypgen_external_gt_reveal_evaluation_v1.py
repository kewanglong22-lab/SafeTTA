...
VERSION = "2026-08-18-S06-D-v1"
BUILD = "S06_D_GT_REVEAL_ONLY_AFTER_DUAL_BACKBONE_NO_LABEL_GLOBAL_LOCK"

ROOT = Path(r"F:\MEDSEG_SAFETTA")
PROTOCOL = ROOT / "docs" / "S06_D_polypgen_external_gt_reveal_evaluation_protocol_v1.md"
EXPECTED_PROTOCOL_SHA256 = "c8d85f21e6382861b3d5950b63fdba7a1b8f74b87eaab8903e9deac484ede5ca"
...
OUTPUT_DIR = ROOT / "outputs" / "S06_D_polypgen_external_gt_reveal_evaluation_v1"
...
def validate_inputs():
    ...
    return rows, lock, global_sha, eligibility_sha
...
def load_mask_gt(zf: zipfile.ZipFile, mask_member: str, external_id: str):
    # FIRST authorized PolypGen target-mask byte access occurs here.
    ...
    if gray.ndim != 2 or gray.size == 0:
        raise RuntimeError(f"Invalid target mask decode: {external_id} shape={gray.shape}")
    maxv = float(gray.max())
    if not math.isfinite(maxv) or maxv <= 0:
        raise RuntimeError(f"Empty/nonpositive target mask: {external_id}")
    gt = (gray > 0.5 * maxv).astype(np.uint8)
    fg = int(gt.sum())
    if fg <= 0:
        raise RuntimeError(f"Thresholded target mask empty: {external_id}")
...
def metrics_from_logit(logit, gt):
    ...
    dice = float((2.0 * inter + 1e-7) / (pred_n + target_n + 1e-7))
...
def run(args):
    eligibility_rows, global_lock, global_sha, eligibility_sha = validate_inputs()
    ...
    per_fields = [
        "architecture", "seed", "external_id", "center", "image_member", "mask_member",
    ...
    write_csv(mask_path, mask_manifest, [
        ...
        "mask_width", "mask_height", "mask_foreground_pixels",
    ])
    summary_fields = [
    ...
    result_lock = {
        ...
        "s06c_global_no_label_lock_sha256": global_sha, "external_images": EXPECTED_IMAGES,
    ...
    lines = [
        ...
        f"  unique masks opened={len(mask_manifest)}", "  prediction recomputation after GT=NO",
